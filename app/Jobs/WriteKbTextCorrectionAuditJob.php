 ...
 * first — the sync attempt inside `approveCorrection()` can fail AFTER the
 * INSERT actually committed (e.g. the connection was cut acknowledging
 * success), and a retry-happy write here must not double the audit trail
 * for the same correction. A duplicate here would be a nuisance a human
 * can de-duplicate by candidate_id; a lost row cannot be recovered without
