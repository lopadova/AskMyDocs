...
- **Come testarlo con i 3 dataset**: crea una collection "Spedizioni urgenti" con `project`/criteri orientati a rotta-logistics e un semantic prompt sul tema; il contatore di anteprima deve riflettere solo documenti logistici. Aggiungi come membro l'`id` di un documento di rotta-logistics. **Check di isolamento**: tentando l'anteprima con un prompt da catalogo scarpe la collection del tenant logistico non deve "pescare" documenti di Passolibero; e una collection creata mentre operi sul tenant logistico non deve mai elencare membri di Prometeo. (Le collection sono per-tenant: due tenant possono legittimamente avere lo stesso slug.)
...
- **Come testarlo con i 3 dataset**: dopo l'ingest dei tre dataset, attendi che gli `analyses` AI vengano calcolati. Le card devono comparire con il `project_key` corretto. **Check di isolamento**: usando il filtro/leggendo le card, le analisi di un documento di rotta-logistics devono citare come *impacted docs* e *cross-references* solo documenti logistici — mai documenti normativi antincendio o di catalogo scarpe. Filtra **Failed** per isolare eventuali analisi non riuscite.
...
- **Come testarlo con i 3 dataset**: da admin, con il contesto su rotta-logistics, premi **Get suggestions from my data**: le proposte AI devono basarsi sui documenti di logistica (es. "Estrai SLA per vettore"). Salva una proposta con **Save this** → compare nello scope **Mine**. **Check di isolamento**: i suggerimenti generati nel contesto del tenant logistico non devono proporre workflow basati su contenuti antincendio o calzaturieri; un workflow salvato per un tenant non deve apparire nello scope di un altro tenant. Verifica che da `viewer` solo il bottone **Hide** sia presente, non Create/Suggest.
...
| Collections | sì (per-tenant) | Anteprima + membri non pescano da altri tenant |
...
| Workflows | sì (per-tenant) | Suggerimenti basati solo sui dati del tenant |
