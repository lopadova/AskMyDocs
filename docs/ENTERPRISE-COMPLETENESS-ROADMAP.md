...
|---|------|---------------|-------|--------|
| R5 | **Google Slides chunking** | `OfficeDocChunker::chunkSlidesDeferred()` emits ONE placeholder chunk (`skip_reason=gslide-deferred`); `drive_gslide` docs are indexed but not usefully searchable. **Re-scoped 2026-09-26 (v8.39 closure) onto `OcrConverter` (v8.36, [ADR 0029](adr/0029-v836-ocr-converter-drivers-and-ocr-provenance.md)):** the connector currently exports a presentation through `VendorMarkdownPassthroughConverter` as plain text (Drive's `text/plain` export), which has no reliable per-slide boundary to split on — that's *why* chunking is deferred, not an OCR gap. `OcrConverter`'s pluggable drivers (docling / mistral-ocr / vision-llm / tesseract) now exist as reusable infrastructure, so the remaining scope is narrower than originally scoped: (1) export each slide as an image/PDF via the Slides API (`presentations.export`, one page per slide) instead of plain text, (2) route that PDF through the EXISTING `OcrConverter` for per-slide extraction — no new OCR engine to build, (3) fetch speaker notes separately via the Slides API's own text endpoint (not an OCR problem at all, a distinct small task). | S/M (down from M/L — the OCR half is no longer new work) |
