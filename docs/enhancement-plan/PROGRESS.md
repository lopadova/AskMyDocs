...
- [x] `README.md` — new "Enterprise admin surface" section with SPA
      route map, Sanctum stateful SPA + Bearer auth shape, and
      screenshot placeholders wired to `resources/screenshots/`.
- [x] `CLAUDE.md` — softened "chat-only" framing to reflect the full
      admin surface; "Critical components" table extended with
      CommandRunnerService / AiInsightsService / admin controllers /
      SPA entrypoint.
- [x] `.github/copilot-instructions.md` — mirror of CLAUDE.md
      updates (R9 discipline).
- [x] `.env.example` — every `env(...)` key in `config/*.php`
      already mirrored with a safe default + comment. ADMIN_* keys
      (Phase H2) + no INSIGHTS_* keys (Phase I is pure DB-backed).
- [x] `docs/enhancement-plan/PROGRESS.md` — PR14 ✅, PR15 ✅.
- [x] `docs/enhancement-plan/LESSONS.md` — PR15 bullets for the
      next maintainer.
- [x] `frontend/e2e/admin-journey.spec.ts` — ONE 10-step golden-path
      scenario (login → dashboard → users → roles → KB tree + source
      edit → graph → logs → maintenance → insights → logout).
- [x] `docs/performance-audit.md` — Lighthouse targets + running
      procedure + current-status TBD cells against `/app/chat` and
      `/app/admin`.
- [x] `resources/screenshots/README.md` — manifest contract for
      every screenshot referenced by README / PR template.
- [x] COPILOT-FINDINGS regression check — `(pending)` cells on
      PR #28 (Phase H1) filled with real fix SHA `39719b9`; PR #29
      (Phase H2) 8-row block added with `59d95bc` + `security` tag;
      frequency snapshot rebuilt against PR#16→#29.
- [x] R13 gate: `bash scripts/verify-e2e-real-data.sh` → OK.
- [x] PHPUnit baseline: 701/701 green (no new tests this PR).
- [x] Vitest baseline: 144/144 green (no new unit tests).
- [x] Playwright `--list`: 62 baseline + 1 new journey = 63.
- [x] Vite build green — main chunk 1.26 MB / 393 KB gz
      (matches Phase I baseline — this PR added zero FE code).
- [x] Commit on branch, push, `gh pr create` targeting
