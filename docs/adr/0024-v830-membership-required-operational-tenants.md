...
`X-Tenant-Id` header. `default` is a legacy slug, not an implicit grant.
...
active tenant memberships. Existing implicit access to legacy `default` data is
materialized once as explicit memberships by an idempotent, audited migration.
The migration does not grant access to any non-default tenant.
...
- Tenant-local identity mutation is refused when the identity is shared with a
  different tenant.
- Deployments retain legacy `default` usability through explicit, auditable
  memberships rather than an invisible universal grant.
