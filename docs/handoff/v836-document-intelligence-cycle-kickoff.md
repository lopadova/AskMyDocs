...
- **A storage-key reservation held from the READ/CONVERT phase, not only from the commit (recorded during W2).** An ingest reads its source and converts it — an OCR run takes minutes — before it takes the storage key's lock, so in that window the file has no row and no holder: `kb:prune-orphan-files` sees an ordinary orphan and can delete it out from under the conversion (a `reference_only` row then has neither original nor artifact; a `full_copy` row no longer satisfies its contract). Mitigated today by an in-flight grace on the sweep (`KB_ORPHAN_SOURCE_GRACE_SECONDS`, default 3600 s: a source younger than that is never an orphan to decide), which narrows but does not close it — and note the grace keys on when the BYTES were written, not on when the conversion started, so it does not cover a first ingest of a file staged earlier at all (`kb:ingest-folder` over a corpus copied days ago, a job that waited in a backed-up queue): those entry points are exposed for the whole read/convert phase. Closing it needs a reservation taken before the read and held (or refreshed) through the commit, in the shape `OcrFigureStore` already uses for its runs; the cost to weigh is a per-key serialization of conversions and a TTL that must outlive `KB_OCR_JOB_TIMEOUT`.
- **A database-level storage-key lock, so the row's visibility and the key are serialized by one authority (recorded during W2).** The cache lock is held across the commit and asserted inside the transaction, but Laravel issues the COMMIT when the closure returns: between the assertion and the COMMIT the row is still invisible to a concurrent `markdown_only` drop or orphan sweep, which could take the key if the TTL lapsed in that stretch. The window is detected and reconciled today (`DocumentIngestor::reconcileIfKeyLockLapsedAcrossCommit()`: the original is probed, the row stamped `source_dropped` when its artifact stands in, the loss reported at `error` when nothing does) but not prevented — a cache lock cannot make an uncommitted row visible. Closing it needs a key lock taken INSIDE the transaction (a `SELECT … FOR UPDATE` on a dedicated key row, or a PostgreSQL advisory lock) so the drop's reference scan blocks until the commit lands. Note the driver spread: SQLite (tests) has neither, so the design must degrade to today's behaviour there.
- **A streaming orphan sweep, so `kb:prune-orphan-files` needs no candidate cap (recorded during W2).** The DB lookups are batched (R3), but the CANDIDATE lists the walk accumulates — orphan sources, `.ocr/` trees, runs — grow with the disk: roughly one entry per converted document, so a large shared disk could make the nightly sweep's memory proportional to the whole listing. Bounded today by `KB_ORPHAN_SCAN_MAX_ITEMS` (default 50 000): a sweep that hits the cap acts on what it collected, reports `scan_truncated=1` and exits non-zero, and the next run continues — correct and observable, but it means a very large disk is swept over several runs. Removing the cap needs the deletions streamed in bounded batches during the walk, which changes the `--dry-run` contract (it lists every candidate today) and the tree/run passes, which judge a run against the rows of the whole scan.
