...
|---|---|---|---|
| W4.1.A — composer + config + .env scaffold | #103 | `6a8e978` | `composer require padosoft/laravel-pii-redactor:^1.1` (moved from `require-dev` in pre-W4.1 spike), `config/kb.php` ships the `pii_redactor` integration block (5 knobs, all default-false), `.env.example` carries the `KB_PII_*` + `PII_REDACTOR_*` lines (commented-out per fail-safe), `bootstrap/providers.php` lists `PiiRedactorServiceProvider` explicitly as a Windows / Herd auto-discovery safety net |
