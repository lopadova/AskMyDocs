...
| W3.2 — atomic chat swap onto `useChatStream()` | #89 | `dd8ca5c` | `ChatView` + `MessageThread` + `Composer` + `MessageBubble` migrated in a single atomic commit; `use-chat-mutation.ts` deleted; 60+ testids preserved; `frontend/e2e/chat-visual.spec.ts` ships 22 pixel-level `toHaveScreenshot({ maxDiffPixels: 0 })` assertions (15 core states + 7 supplementary) — baseline PNGs are generated by Playwright at first run and live next to the spec, not committed under the repo |
...
- 10/10 CI checks GREEN on every PR at merge time. The `tests.yml` workflow defines 5 jobs (3 PHPUnit on the PHP 8.3 / 8.4 / 8.5 matrix, 1 Vitest, 1 Playwright E2E) and runs each one twice per PR — once on the `push:` trigger and once on the `pull_request:` trigger — for a total of 10 status checks on the rollup
- Copilot Code Review converged to **0 outstanding must-fix** on every PR after the R36 loop:
  ...
  - PR #89 (W3.2 atomic swap): **13 cycles** — the highest single-PR cycle count in v4.0 to date, driven by the swap surface (4 components + adapter layer + 22 visual screenshot assertions all interacting)
...
- **R36 cycles cluster around swap surface** — PR #89 hit 13 cycles because the atomic swap touched four components plus the adapter layer plus 22 visual screenshot assertions simultaneously. Future high-fan-out swaps should bundle the foundation PR earlier (PR #88 pattern) so the swap PR has only the orchestration concern, not the type-shape concern.
...
- **Pixel-level snapshots ARE the design-fidelity contract** — `toHaveScreenshot({ maxDiffPixels: 0 })` on the 22 assertions in `chat-visual.spec.ts` (15 core states from `PLAN-W3` §7.4 plus 7 supplementary) caught two regressions during the R36 loop on PR #89 that the testid + ARIA assertions had let through. Recommendation for any future "preserve UX bit-for-bit" migration: snapshots are mandatory, not nice-to-have.
