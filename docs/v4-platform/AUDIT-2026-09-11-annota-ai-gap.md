...
- AskMyDocs column grounded by `git grep` on `origin/main` at **v8.35.0**
  ...
  every in-scope ❌ carries the grep that came back empty and what would be
  needed — the one out-of-domain row (image labelling / COCO / YOLO) is
  excluded by §3 rather than grepped;
...
| **Content export as a portable workspace** (Markdown ZIP · LLM Wiki with `AGENTS.md`/`CLAUDE.md` skills · Obsidian-compatible) | ❌ | ✅ | `ConnectorExportCommand` exports connector *configuration*. The Auto-Wiki (v8.11) — hub, indices, `log`, lint, BFS — lives in Postgres and is reachable through its Artisan commands (`kb:wiki-index`, `kb:wiki-lint`, `kb:wiki-navigate`), the admin API and MCP (`KbWikiHubTool`, `kb_wiki_indices`) — but never as a portable folder. "Content export/portability" is in the README `Future` row since the Affine gap audit. |
| **MCP tools on the preparation loop** (edit page, set review status, start job, create/get export) | ❌ | ✅ | 48 tools in `app/Mcp/Tools/` — retrieval, canonical, wiki, engagement, FinOps, connectors and runtime/admin surfaces (`AppSettingsTool`, `WidgetIntroConfigTool`, `FlowRunStatusTool`) — none of them on the preparation loop: none lets an agent see or correct a conversion. |
