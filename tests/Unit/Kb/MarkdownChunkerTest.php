...
class MarkdownChunkerTest extends TestCase
{
    ...
    {
        $md = "First paragraph.\n\nSecond paragraph.\n\nThird one.";
        $chunks = $this->chunker->chunkLegacy('file.md', $md);
    ...
    {
        $chunks = $this->chunker->chunkLegacy('auth.md', "Para A\n\nPara B");
    ...
    {
        $chunks = $this->chunker->chunkLegacy('f.md', "A\n\n\n\nB");
        $this->assertCount(2, $chunks);
    }

    public function test_returns_empty_collection_for_empty_markdown(): void
    {
        $this->assertTrue($this->chunker->chunkLegacy('f.md', '')->isEmpty());
        $this->assertTrue($this->chunker->chunkLegacy('f.md', "   \n\n   ")->isEmpty());
    }

    public function test_trims_whitespace_in_chunk_text(): void
    {
        $chunks = $this->chunker->chunkLegacy('f.md', "  hello  \n\n  world  ");
    ...
    {
        $chunks = $this->chunker->chunkLegacy('runbook.md', "# S\n\nBody");
