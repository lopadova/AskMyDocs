...
const logger = createLogger('SseMcpClient');
...
export class SseMcpClient extends McpClientBase {
    ...
    async handshake(): Promise<HandshakeResult> {
        ...
        try {
            ...
            try {
                ...
                logger.warn('SSE listTools failed during handshake', { error: this.errorMessage(error) });
            }
            return {
                ok: true,
    ...
    async close(): Promise<void> {
        ...
        try {
            ...
            logger.warn('SSE close failed', { error: this.errorMessage(error) });
        }
