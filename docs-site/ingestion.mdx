...
| Everything else | `default` | (queue connection default) |

Run a worker per queue — e.g.
`php artisan queue:work --queue=connectors`,
`--queue=kb-ingest`, `--queue=default` — or, under Horizon, one
auto-scaling supervisor per queue. The previous default (`CONNECTOR_SYNC_JOB_QUEUE=default`)
