...
- **`kb:prune-archived-versions`** — drop old archived document versions beyond
  the per-family retention cap; since v8.36 each pruned version's conversion
  artifact goes with it, and the run sweeps stale `.tmp` files
  (`KB_CONVERSION_ARTIFACTS_TMP_MAX_AGE`) and artifacts no row references.
