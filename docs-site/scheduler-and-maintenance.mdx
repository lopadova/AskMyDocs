...
- **`kb:prune-archived-versions`** — drop old archived document versions beyond
  ...
  stale `.tmp` files (`KB_CONVERSION_ARTIFACTS_TMP_MAX_AGE`) and artifacts no
  row references on that disk. Every refused removal is counted (`*_failed`)
...
- **`kb:prune-orphan-files`** — remove markdown files on the KB disk with no
  ...
  `kb:prune-archived-versions` use.
