...
- **`kb:prune-orphan-files`** — remove markdown files on the KB disk with no
  ...
  and previews no `kept_meanwhile`: it is a real-run outcome (who holds a key
  at that instant), not something a preview can know. Since v8.36 it is also the reaper of the
