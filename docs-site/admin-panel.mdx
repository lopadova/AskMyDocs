...
- **Users / Roles / RBAC** — six roles (`system-admin`, tenant `super-admin`,
  admin, dpo, editor, viewer), permission matrix and role editor. The protected
  system role is visible only to platform operators and cannot be assigned here.
...
- **KB explorer** — browse the canonical tree, an inline source editor
  ...
  diff / restore any version — since v8.36 over the stored conversion artifact,
  with a *faithful* (both sides stored and verified) / *stored, not verified* /
  *index diff* label and the actor + reason of each version).
...
or endpoint that forgets its `role:` / `can:` gate fails CI. The six roles plus
