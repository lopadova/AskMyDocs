...
- **Separate platform scope from tenant privilege.** `system-admin` owns
  `platform.admin` + `tenant.cross-access`; `super-admin` owns the maximum
  tenant permission set but needs membership. A system administrator also holds
  companion `super-admin` so existing tenant route allowlists remain stable.
...
- **Rename authorizes the *target* team, not the request scope.** Team
  ...
  the `tenant.cross-access` permission (system administrators only) — the exact rule
...
1. `GET /api/admin/teams` returns `acme` (manageable) and the read-only
   `default`. A system administrator additionally sees every active team via
   `tenant.cross-access`; a tenant super-admin sees only memberships.
...
- **Every tenant administrator sees only membership teams.** Cross-team
  visibility requires `tenant.cross-access` (system-admin only). This is why
  **create** attaches the acting user as a member — otherwise an admin could
  create a team they then couldn't see.
