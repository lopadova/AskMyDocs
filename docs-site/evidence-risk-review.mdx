...
- **HTTP API** — `/api/admin/evidence-risk-review/*` (reviews list + detail,
  profiles, taxonomy, submit), host-secured with `tenant.resolve` +
  `auth:sanctum` + `tenant.authorize` + `can:viewEvidenceRiskReview` (R32
  matrix-locked). `tenant.resolve` runs first so `TenantContext` is set before
  the bound `TenantResolver` stamps + scopes the review-log rows (R30).
