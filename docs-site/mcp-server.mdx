...
- **Write surfaces** for auto-wiki operations: evidence tier (`KbSetEvidenceTierTool`),
  ...
  (`KbWikiPromoteTool`).
- **Document-intelligence read surface (v8.36):** `KbOcrStatusTool` reports whether a
  document was OCR'd, with which driver, per-page confidence and figure count — read-only
  by design; an agent cannot start an OCR run or spend (see
  [Documents & OCR](/documents-and-ocr)). `KbDocumentVersionsTool` lists a document's
  version family — status, live flag, who created each version and why, the artifact
  content hash, whether a stored conversion artifact is readable and verified
  (`has_artifact`) and its verified state (`artifact_state`) — newest first,
  bounded and paged (`limit`, at most `KB_VERSIONS_TIMELINE_LIMIT`, `offset`
  skips the newest; `total` is the family size, `truncated` says when it holds
  more than the page reaches; an invalid page is an error, never a silently
  satisfied request) — metadata only: the
  artifact content and the artifact-aware diff stay on the role-gated HTTP surface
  because the artifact is un-redacted converter output (a documented R44 exception).
