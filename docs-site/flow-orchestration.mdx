...
Both branches return the **same key set**, so a caller parses one shape and
reads `persistence_enabled` to interpret it. Only the values change, and the
counts come back **null rather than zero**: zero is a measurement, and a
dashboard renders *0 failed* as healthy — the opposite of what an unrecorded
corpus means.
