...
erDiagram
    ...
    chat_folders ||--o{ conversations : "nullOnDelete (unfiles)"
    knowledge_documents {
      bigint id PK
      string tenant_id
      string project_key
...
`uq_kb_doc_tenant_version` (the idempotency anchor),
`(tenant_id, project_key, doc_id)` = `uq_kb_doc_tenant_doc_id`,
`(tenant_id, project_key, slug)` = `uq_kb_doc_tenant_slug`. All three start
with `tenant_id` since
`2026_10_02_000011_tenant_scope_knowledge_document_uniques.php`: `project_key`
is not a tenant boundary, so keying row identity on it alone left the indexes
disagreeing with every tenant-scoped read and write.
...
Session organisation for the [Sessions workspace](/chat-sessions-workspace)
adds four columns to `conversations`: `chat_folder_id` FK **nullOnDelete**
(never cascade — deleting a folder unfiles its sessions, it does not destroy
chat history), `pinned_at?`, `archived_at?` and `importance`
(`normal | high | critical`). Indexes `(tenant_id, user_id, archived_at)` —
the sidebar's only selective predicate — and `chat_folder_id`, because
`nullOnDelete` sweeps conversations by it on every folder delete.

`pinned_at` / `archived_at` are timestamps rather than booleans, so the
request contract is asymmetric: clients send `{"pinned": true}` and read
back the timestamp. Organisation writes run with `timestamps = false`,
because `updated_at` is both the recency ordering key and the displayed
"last activity" — a pin must not reorder the list.

### `chat_folders`

User-created folders grouping chat sessions. `tenant_id`, `user_id` FK
(cascade), `name`, `position`, timestamps. Unique
`(tenant_id, user_id, name)` = `uq_chat_folders_tenant_user_name` — R31, so
two tenants and two users in one tenant may each keep an "Issue 42". A
folder is private to one user: there is no sharing, no nesting, and
authorization lives in `ChatFolderService` (`forTenant()` +
`where('user_id')`), not in a policy or a global scope — the same posture as
`chat_filter_presets`.
