...
1. **Idempotency by content hash.** A document's identity is the tuple
   `(tenant_id, project_key, source_path, version_hash)` where `version_hash` is the
...
- If a row with the same `(tenant_id, project_key, source_path, version_hash)` exists → no-op.
...
uniques `(tenant_id, project_key, doc_id)` / `(tenant_id, project_key, slug)`
