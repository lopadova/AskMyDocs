...
<Warning>
One glob shape has no exact translation: a glob that mixes a single-segment
wildcard **and** a cross-segment wildcard. The single-segment bound cannot be
expressed by a whole-string separator count.

Such a glob **grants nothing**. Widening it to its literal prefix would be the
wrong direction here — a superset is harmless only when a later gate narrows
it, and retrieval has none, so the extra documents would reach the model as
grounding and be cited. Failing closed can hide a document the member may
legitimately read, which is the side to err on when the control cannot be
expressed at all.

Prefer allowlist entries using one wildcard kind or the other, or split the
glob into exactly-expressible entries; `ScopeAllowlistSql::isExact()` reports
