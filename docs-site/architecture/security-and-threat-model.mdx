...
| Role | Scope |
|---|---|
| `system-admin` | global platform control (`platform.admin`, `tenant.cross-access`) plus companion `super-admin` |
...
Only `system-admin` receives `tenant.cross-access`. A tenant `super-admin`
therefore sees and enters only tenants represented by its memberships. The
global registry is separately gated by `platform.admin`; see
