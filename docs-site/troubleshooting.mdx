...
- **Admin dashboard health panel** — backed by
  `App\Services\Admin\HealthCheckService`, it returns a per-subsystem status
  map: `db_ok`, `pgvector_ok`, `queue_ok`, `kb_disk_ok`,
  `embedding_provider_ok`, `chat_provider_ok`, plus a `pii_redactor` block and a
  `checked_at` timestamp. Each subsystem reports `ok | degraded | down`.
