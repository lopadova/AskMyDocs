...
- A never-configured preference (omitted or `null` sections) resolves to **all**
  ...
  opt-out). Don't conflate the two — the UI sends `null` when every section is
  selected (so the choice stays "all" and any future catalog addition is
  auto-included), and `[]` only when the user explicitly unchecks every box.
