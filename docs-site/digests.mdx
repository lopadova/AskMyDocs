...
- **HTTP:** `GET /api/admin/digest/preview` (admin, RBAC-gated); `GET /api/me/digest/latest` and `GET` / `PUT /api/me/digest-preferences` (per-user).
...
A user opts out of everything but the weekly review queue:

```bash
curl -s -X PUT -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d '{"frequency":"weekly","sections":["stale_review"]}' \
...
- An empty per-section preference array means **none** (explicit opt-out); a
  never-configured preference (null) means the tenant default. Don't conflate the
  two.
