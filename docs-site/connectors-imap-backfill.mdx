...
- a terminal window failure is isolated while the remaining windows continue;
...
| `running` | The pump is claiming and importing bounded windows, skipping any terminally failed window. | `completed` after every window completes; `failed` after all remaining windows finish when at least one window failed. |
| `completed` | All snapshot windows finished. | Terminal; a later incremental sync starts from `cutoff_at`. |
| `failed` | Discovery or a window exhausted its real-error budget. | Resume the same campaign from saved checkpoints; restart only when `UIDVALIDITY` invalidated the snapshot. |
| `paused` | Reserved durable state; the scheduler does not pump it. | Operator-controlled. |

`WithoutOverlapping` uses the same physical-mailbox key as incremental sync and
explicitly shares that key across the sync, discovery, and import job classes.
Lock contention therefore releases discovery/import jobs back to the queue before
they open another IMAP connection, without consuming the real-exception budget;
`retryUntil()` bounds those releases by the configured wall-clock requeue window,
while `maxExceptions` still fails repeated real exceptions.
...
When one import job exhausts its real-error budget, its window becomes `failed`
and immediately dispatches the pump again. Other pending windows therefore keep
advancing. The campaign becomes `failed` only after no pending or active window
remains, preserving a complete list of the windows that need an operator resume.
...
installation row and return the existing `discovering` or `running` campaign. If
the latest campaign failed during discovery, the same campaign ID retries
discovery. If it failed while importing, completed windows remain untouched,
incomplete windows return to `pending`, aggregate counters are rebuilt from their
durable rows, and the pump continues from each saved `last_uid`. Only an
`UIDVALIDITY changed` failure creates a new snapshot because the old UIDs no
longer identify the same mailbox contents.
...
5. With `batch_size=10` and `fetch_size=5`, each import job searches at most 11
   UIDs, fetches at most two 5-message chunks, persists them and commits the next
   UID checkpoint. A separate execution cap also splits campaigns created with a
   larger historical batch size.
6. Poll the GET endpoint. Worker restarts do not reset completed windows; stale
   work is recovered by the next scheduler sweep. If a real failure exhausts its
   retries, that window is skipped while the other windows continue. After the
   queue is drained, **Resume full import** reuses the same campaign and continues
   failed windows from their last committed UID rather than returning to zero.
...
| `CONNECTOR_IMAP_BACKFILL_BATCH_SIZE` | `10` | Messages checkpointed per import job for new campaigns. |
| `CONNECTOR_IMAP_BACKFILL_MAX_MESSAGES_PER_JOB` | `10` | Runtime cap applied to new, running and resumed campaigns. |
| `CONNECTOR_IMAP_BACKFILL_FETCH_SIZE` | `5` | Bodies fetched per IMAP bulk exchange. |
...
- **UIDVALIDITY changes invalidate the snapshot.** The current window fails rather
  than applying an old UID checkpoint to a rebuilt mailbox; status exposes
  `retry_mode: restart` and the UI creates a fresh campaign. Other failures expose
  `retry_mode: resume` and retain the existing checkpoints.
- **Folder selection is captured at start.** Changing include/exclude settings does
  not rewrite an active or ordinarily resumed campaign. A restart caused by an
  invalidated UID namespace captures a new snapshot and current settings.
