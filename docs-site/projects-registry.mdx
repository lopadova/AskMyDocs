...
The scope allowlist is enforced in SQL, so it constrains **retrieval** as well as
listing — a member scoped to `hr/policies/**` cannot receive `hr/salaries/**`
chunks as chat grounding. See
[scope allowlists reach the retrieval path](/architecture/security-and-threat-model)
for the glob translation and its one inexact shape.
