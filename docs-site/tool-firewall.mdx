...
**It ships on.** ADR 0028 specified `OFF` for this phase and the product owner
reversed that, which is recorded in the ADR rather than left as a discrepancy
between a document and a config file.

The ADR's caution stands and is worth knowing before you upgrade: a deployment
already ingesting email will find that **turns grounded in a message stop being
able to act**. What decided it the other way is that a security control shipping
off protects nobody until somebody remembers to switch it on — and the chain
above is not hypothetical, it is three ordinary facts about the product with no
boundary between them.

The failure direction is the safe one: the turn declines to act rather than
acting on a stranger's instructions, and the answer still comes back from the
same context with the same citations. Setting the variable to `false` restores
