...
Session login and bearer-token issuance both require an active account.
Email input is trimmed/lower-cased and resolved through the case-insensitive
global identity key.
...
| `GET` | `/api/kb/tree` | `auth:sanctum` + `token.ability:kb:read` | reader-side document tree |
...
| Area | Base path | Auth |
|---|---|---|
| Chat sessions | `/conversations` (CRUD, **no `/api` prefix** — session-auth AJAX) | `auth` + `tenant.authorize` |
| Chat folders | `/api/chat-folders` (index/store/update/destroy) | `auth:sanctum` |
...
**`GET /conversations`** — the acting user's sessions in the active tenant,
ordered pinned-first (most recently pinned first), then by importance, then by
recency. Optional `?archived=` selects the slice: absent → active only (the
default, so the plain call is unchanged), `1` → archived only, `all` → both. An
unrecognised value falls back to *active* rather than widening the listing.

The response is a **bare JSON array**, not a `{data: …}` envelope — it always
has been, and wrapping it would be a breaking change, so extensions here are
additive only. Each row:
`{ id, title, project_key, chat_folder_id, pinned_at, archived_at, importance,
created_at, updated_at }`.

**`PATCH /conversations/{id}`** — rename and/or organise, applying whichever of
`title`, `chat_folder_id`, `pinned`, `archived`, `importance` are present. Note
the deliberate asymmetry: pin and archive are **booleans in, timestamps out**,
so a client never invents a date. `chat_folder_id: null` unfiles the session,
which is different from omitting the key (leave the folder alone). An **empty
body is a 422**, not a 200 no-op. `chat_folder_id` must name a folder the acting
user owns in the active tenant — otherwise 422, never a cross-file. A session in
another tenant is a 404 (no existence oracle); another user's session is a 403.

Organisation writes do **not** touch `updated_at`, because it is both the
recency ordering key and the displayed "last activity"; a rename still does.
See the [Sessions workspace](/chat-sessions-workspace).
...
| KB OCR (v8.36) | `GET /api/admin/kb/documents/{id}/ocr` (status), `POST …/ocr` (re-run, 202), `GET /api/admin/kb/uploads/{batch}/estimate` (pre-commit cost estimate) | `role:admin\|super-admin` |
| Time Machine artifacts (v8.36) | `GET /api/admin/kb/documents/{id}/versions/{versionId}/content` (a version's stored Markdown, or its chunk reconstruction, with `source`); `…/versions` is bounded and paged (`?limit=` 1..`KB_VERSIONS_TIMELINE_LIMIT`, `?offset=` newest skipped, 422 on an invalid value; additive `meta.limit` / `meta.offset` / `meta.truncated`, `meta.total` is the family size) and, with `…/versions/diff`, gains additive `version_actor` / `version_reason` / `content_hash` / `has_artifact` (true only for a readable artifact whose bytes hash to `content_hash`, never the pointer alone) / `artifact_state` (`none` · `verified` · `unverified` · `missing` · `mismatch`) / `restored_by` / `restored_at`, `from_source` / `to_source` and the integrity verdicts `from_integrity` / `to_integrity` (`verified` · `mismatch` · null); the content endpoint carries `integrity` too | `role:admin\|super-admin` |
...
## System-admin tenant control — `/api/system-admin/tenants`

These endpoints require `can:platform.admin` and are intentionally global (no
active-tenant header). A tenant `super-admin` does not satisfy this capability:

| Method | Path | Purpose |
|---|---|---|
| `GET` | `/api/system-admin/tenants` | Paginated registry; `search`, `status`, `page`, `per_page` |
| `POST` | `/api/system-admin/tenants/availability` | Non-mutating slug + account + requested-role preflight |
| `POST` | `/api/system-admin/tenants` | Atomic tenant + project + create/attach user + role + membership |
| `GET` | `/api/system-admin/tenants/{slug}` | Tenant metadata plus paginated users, roles, permissions and project access |
| `POST` | `/api/system-admin/tenants/{slug}/lifecycle-preview` | Single-use confirmation bound to the exact status transition |
| `PATCH` | `/api/system-admin/tenants/{slug}` | Change display name and/or confirmed `active\|suspended\|archived` lifecycle |

See [System administration](/system-administration) for bootstrap, request
behavior, duplicate handling and lifecycle enforcement.

## Embeddable widget — `/api/widget`

The KITT widget authenticates with a `widget.key` (`throttle:120,1`):

| Method | Path | Purpose |
|---|---|---|
| `GET` | `/api/widget/setup` | resolved skill/theme configuration |
| `POST` | `/api/widget/user-token` | server-side `ik_` exchange for an origin-bound `wu_` |
| `POST` | `/api/widget/session-token` | mint a single-use `wt_` |
| `GET` | `/api/widget/sessions` | paginated identity history (`wu_` required) |
| `GET` | `/api/widget/sessions/current` | newest open identity session, or `204` |
| `POST` | `/api/widget/sessions/start` | start a conversation |
| `POST` | `/api/widget/sessions/{id}/step` | continue a conversation |
| `POST` | `/api/widget/sessions/{id}/exec-tool` | run an enabled backend tool |
| `POST` | `/api/widget/sessions/{id}/cancel` | abort a conversation |
| `GET` | `/api/widget/sessions/{id}/replay` | replay identity-scoped visible steps |

`/sessions/current` filters `active`, `waiting_user` and `waiting_tool`, ordered
by `updated_at DESC, id DESC`; tenant, key, project and pseudonymous identity
are all resolved server-side.

Admin identity mutations use `PATCH /api/admin/widget-keys/{id}` (enable/disable)
or `POST /api/admin/widget-keys/{id}/rotate-identity-secret`. Both carry
`identity_credential_version`; a stale version returns `409
identity_credential_conflict`. Secrets are returned only once. See
[KITT widget](/kitt-widget).
