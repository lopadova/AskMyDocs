...
## Super-admin tenant control — `/api/super-admin/tenants`

These endpoints require the exact `super-admin` role and are intentionally
global (no active-tenant header):

| Method | Path | Purpose |
|---|---|---|
| `GET` | `/api/super-admin/tenants` | Paginated registry; `search`, `status`, `page`, `per_page` |
| `GET` | `/api/super-admin/tenants/availability` | Non-mutating slug + account preflight |
| `POST` | `/api/super-admin/tenants` | Atomic tenant + project + create/attach user + role + membership |
| `GET` | `/api/super-admin/tenants/{slug}` | Tenant metadata plus paginated users, roles, permissions and project access |
| `PATCH` | `/api/super-admin/tenants/{slug}` | Change display name and/or `active\|suspended\|archived` lifecycle |

See [Team and tenant management](/team-management) for request behavior,
duplicate handling and lifecycle enforcement.
