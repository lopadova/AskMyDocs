...
| `POST` | `/api/auth/token/revoke` | `auth:sanctum` — deletes the caller's PAT (`204`) |
...
| `GET` | `/api/kb/documents/{id}/preview` | `auth:sanctum` + `token.ability:kb:read` | document source preview |
...
sessions** (which carry a `TransientToken`, governed by the route's RBAC stack),
so these dual-auth routes serve both transports without breaking either.
