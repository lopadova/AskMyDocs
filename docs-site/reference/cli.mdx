...
- **Retention overrides.** Prune commands accept `--days=N`; `0` disables that
  rotation.
- **Path arguments** are resolved through `KbPath` and (for folder walks)
  relative to `KB_PATH_PREFIX`.
...
| `notifications:digest-weekly` | Email weekly notification digests |
