...
| `kb:artifacts-backfill {--project=} {--tenant=default} {--dry-run}` | v8.36 — store the conversion artifact of live rows that have none; writes only when the reconversion hashes to the recorded `document_hash` (`hash_mismatch` writes nothing), `reference_only` rows are `intentionally_missing`. Operator-only (R44 exception). |
...
| `kb:prune-archived-versions {--tenant=} {--keep=} {--dry-run}` | Drop old archived versions beyond the cap; since v8.36 also removes each pruned version's conversion artifact and sweeps stale `.tmp` files + orphan artifacts |
