...
- **`KB_INGEST_PII_STRATEGY` must match exactly.** Accepted values are `mask`
  (default) and `tokenise` (with a trailing `s`). When the connector ingest
  redaction boundary is enabled (`KB_PII_REDACTOR_ENABLED=true` +
  `KB_CONNECTOR_INGEST_PII_REDACT=true`), any other value — including the common
  typo `tokenize` — throws an `InvalidArgumentException` at ingest time so the
  misconfiguration surfaces immediately (R14), rather than silently masking.
  While the boundary is OFF, `redactContent()` returns the original untouched and
...
| `tokenise` | `tokenise` | Reversible surrogate in KB; original in per-tenant vault. Requires `PII_REDACTOR_SALT`. |
| anything else | — | `InvalidArgumentException` thrown at ingest time (R14). |

The strategy applies at the **connector ingest boundary**
(`HostIngestionBridge::redactContent`) when `KB_CONNECTOR_INGEST_PII_REDACT=true`.
Extending tokenisation to the inline `DocumentIngestor` (HTTP/CLI) path is a
follow-up in this cycle.

Detokenisation today is exposed for **chat-log** rows via the Log Viewer
endpoint (`POST /api/admin/logs/chat/{id}/detokenize`) and the PII Redactor
Admin SPA cross-mounted at `/app/admin/pii-redactor` (both RBAC-gated and fully
audited). A dedicated **KB-document** detokenise surface — re-identifying an
ingest-tokenised document on demand — is a follow-up in this cycle.
