...
`POST /api/kb/ingest` batch and the `kb:ingest-folder` CLI, both via
`IngestDocumentJob → DocumentIngestor`. Each **chunk's text** is rewritten
through the active strategy **before** it is hashed, embedded, and persisted, so
the vector store + `chunk_text` never hold raw PII while the per-tenant vault
keeps the originals. Redaction is deliberately scoped to the chunk text, not the
whole markdown: the raw markdown stays the idempotency anchor (`document_hash`)
and the canonical frontmatter stays parseable. Tokens are deterministic, so a
re-ingest of identical content yields identical surrogates and remains
idempotent (same `chunk_hash`).
