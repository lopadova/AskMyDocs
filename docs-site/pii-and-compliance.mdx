...
mask/hash/drop — nothing was minted to reverse), is **tenant-scoped** (R30 — an
operator cannot re-identify another tenant's document by id), and writes an
immutable `admin_command_audit` row (`command='pii.detokenize'`,
`status=completed|rejected`, with the `surface`) for **every** unmask attempt.
