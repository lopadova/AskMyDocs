...
| **Campaigns** | list + create/edit with the **multi-tenant grant editor** (a Spatie role + KB projects across one or more tenants); `key` + `type` are immutable on edit; `super-admin` is rejected client- and server-side | `GET/POST …/campaigns`, `PATCH …/campaigns/{id}`, `GET …/tenants` |
...
- **Campaign creation writes a grant — mind super-admin + immutability.** The
  campaign builder's grant editor provisions a Spatie role + KB projects across
  one or more tenants on redemption; `super-admin` is rejected (client *and*
  server) and a campaign's `key` + `type` are immutable after creation (the
