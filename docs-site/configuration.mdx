...
| `config/agent.php` | Planner mode, router/catalog bounds, loop and evidence budgets |
| `config/connector-mcp.php` | MCP runtime rollout, OAuth, SSRF allow-list, sync and MCP App limits |
...
Each provider has a `<NAME>_API_KEY`, `<NAME>_CHAT_MODEL`, `<NAME>_TEMPERATURE`,
...
is documented in [AI providers](/ai-providers).
...
The planner and MCP connector have independent rollout switches:

```bash
AGENT_PLANNER_MODE=classic          # classic | shadow | capability
AGENT_ROUTER_CATALOG_LIMIT=40
AGENT_PLANNER_CANDIDATE_LIMIT=8

MCP_CONNECTOR_ENABLED=false         # deployment-wide hard switch
MCP_CONNECTOR_RUNTIME_MODE=off      # off | shadow | active, tenant-overridable
```

`classic` is intentionally the unchanged legacy planner path. In `shadow`, only
classic actions execute and the capability planner writes a masked comparison.
`capability` executes a read-only, schema-validated plan. See the
[agent capability planner](/architecture/agent-capability-planner) for routing,
validation, selection continuity and promotion gates. MCP connector rollout and
rollback are documented in the repository runbook `docs/mcp-connector-rollout.md`.
