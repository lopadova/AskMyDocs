...
<Steps>
  ...
  <Step title="Seed roles (RBAC)">
    ...
    ```bash
    php artisan db:seed
    php artisan system-admin:grant you@example.com --yes
    ```
  </Step>
...
```bash
KB_FILESYSTEM_DISK=kb       # which configured disk to use (kb | s3 | r2 | …)
KB_DISK_DRIVER=local        # driver of the default "kb" disk (local | s3)
KB_PATH_PREFIX=             # optional prefix every path is resolved under (never a `.artifacts` segment: reserved)
KB_RAW_DISK=kb-raw          # raw (pre-conversion) source retention
```

To serve KB docs from S3, set **`KB_FILESYSTEM_DISK=s3`** (with the standard
...
# 3. first admin
php artisan system-admin:grant ops@acme.com --yes
