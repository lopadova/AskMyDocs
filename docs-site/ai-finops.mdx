...
- **Fixed-precision decimal money (since v8.18, `laravel-ai-finops` v1.3.0).** The
  package now also exposes every money value as a fixed-precision **8-dp decimal
  string** under additive `*_decimal` keys, so small per-token costs survive
  serialisation without float-rounding drift. The host runs `^1.3`; the three MCP
  read tools and `ChatTurnCostResolver` read the new shape while the existing
  numeric keys stay in place (additive — R27), so older clients are unaffected.
