...
- **One core, both paths.** The write lives in `DocumentIngestor`'s shared
  ...
  **lease** on its temp (`KB_CONVERSION_ARTIFACTS_TMP_LEASE`, the primary
  ...
  not a renewal, so the window shrinks to the step itself. A lock that records no
  acquisition owner at all (a third-party store registered with
  `Cache::extend()`; ownership is read by capability, from Laravel's lock or
  from any lock answering `isOwnedByCurrentProcess()`) cannot prove ownership,
  so its guarded step is refused as well — a stop rather than a degradation:
