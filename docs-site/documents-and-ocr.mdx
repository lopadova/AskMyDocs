...
| `KB_ORPHAN_SOURCE_GRACE_SECONDS` | `3600` | Seconds a source file must be untouched before `kb:prune-orphan-files` may consider it an orphan. An ingest reads and converts its source — an OCR run takes minutes — BEFORE it takes the storage key's lock, so in that window the file has neither a row nor a holder and an ordinary-looking orphan may be a conversion's input; a younger file is kept and counted `kept_meanwhile`. `0` disables the grace. It narrows the window without closing it, and it keys on when the BYTES were written rather than on when the conversion started: a conversion slower than the grace is still exposed, and a first ingest of a file staged earlier (a folder sync over a corpus copied days ago, a job that waited in a backed-up queue) gets no protection from it at all. The reservation that would close both is a recorded follow-up. |
