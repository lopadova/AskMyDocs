...
- **One core, both paths.** The write lives in `DocumentIngestor`'s shared
  ...
  **lease** on its temp (`KB_CONVERSION_ARTIFACTS_TMP_LEASE`, the primary
  ...
  move into place. The publish itself runs under the artifact **path's lock**
  after re-checking that the row still points there, and every removal of an
  artifact (hard delete, prune, orphan sweep) re-checks the references under
  the same lock before deleting — an identical ingest that recreated the same
  content-addressed path meanwhile keeps its artifact (`artifacts_kept` for a
  pruned or deleted row, `artifact_orphans_kept` for an orphan candidate).
  The path lock needs a lock-capable cache store (`CACHE_STORE=redis` in
  production): on a store that cannot lock, a publish is refused (thrown and
  its temp discarded) and a removal is reported `failed` — never run
  unguarded — while only the temp lease degrades to the age threshold. The
  `null` store, which implements the lock contract and grants every lock
  without excluding anyone, is detected and refused exactly like a store that
  cannot lock; what stays undetectable is a provider whose locks do not
  exclude ACROSS processes (the array store, process-local by design). Neither
  lock is renewed: every critical section asserts, right before its
  irreversible step, that it still owns its lock (`HeldLock`) — a lapsed TTL
  is a refusal (the commit rolls back, the original is kept, the publish
  discards its temp, the removal is `failed`) — a check right before the step,
  not a renewal, so the window shrinks to the step itself. A lock that cannot prove ownership — ownership is
  read by capability, from any lock exposing a callable
  `isOwnedByCurrentProcess()` that answers a boolean, so a probe that is
  missing, not public, or answers anything else (a third-party store
  registered with `Cache::extend()`) does not qualify — has its guarded step
  refused as well — a stop rather than a degradation:
  every artifact-enabled ingest rolls back and retries and every prune reports
  `failed`, until `CACHE_STORE` names a lock-capable store. Reported once per
  class.
  The **source file** of a hard delete takes the storage key's lock the same
  way: the reference scan, the `.ocr/` purge and the delete all run under it,
  each irreversible step asserting the TTL has not lapsed. Every refusal — a
  key another writer holds right now, a store that cannot lock, a lapse
  mid-section — keeps the file for the orphan sweep and reports
  `file_deleted: false`; the row is already committed, and a stale file never
  fails a deletion. With artifacts off nothing else takes that lock, so the
  hard delete takes none either.
...
- **Restore keeps the creation provenance and records itself apart.**
  ...
  The restored version's **canonical identity is its own**, reconstructed from
  the frontmatter the archive retained and re-validated by the same parser the
  ingest path runs: a restore is the re-ingest of older bytes, so a canonical
  version restored over a non-canonical live one reclaims its slug, and a
  version that never declared one does not inherit the live row's. Only a
  legacy row archived before the frontmatter was persisted still carries the
  outgoing version's identity — there it is the only copy left. If the slug or
  doc_id is already held by another document (an archived sibling, or another
  source path in the same project), the content is restored **without** the
  identity and a warning names the holder — never a failed restore, and never
  a slug taken from its current owner. "Held" is judged the way the unique
  index judges it, not the way this reader may read: a soft-deleted or
  ACL-hidden holder still occupies the slot, so the probe lifts both global
  scopes. It keeps the tenant filter, because the uniques themselves are
  `(tenant_id, project_key, slug)` / `(tenant_id, project_key, doc_id)` — a
  holder in **another** tenant is not a conflict and does not cost this
...
- **`markdown_only` honours every row's contract.** Each row records the mode
  ...
  mode only when a NEW version is being stamped; a row ALREADY persisted with
  an invalid or absent stamp reads as `full_copy`, the conservative mode, and
  keeps its original whatever `KB_SOURCE_RETENTION` says today. Do not expect
  a historical row with a malformed stamp to follow a later change of the
  setting. The drop is gated on the row's **own** stamp, never on the
...
- **Backfill is operator-only and judges each row on its own contract.**
  ...
  chunks); a missing source is reported, not invented. The command exits
  non-zero when any row ended `source_missing`, `conversion_failed` or
  `disk_unavailable` — a row whose artifact it could not produce —
  `hash_mismatch` being informational, and `--dry-run` predicts the same
  exit code. An identical re-ingest with the flag on repairs or publishes a
  version's artifact the same way.
...
| `KB_CONVERSION_ARTIFACTS_TMP_LEASE` | `7200` | Seconds a writer's cache lease on its temp file lives (taken before the temp is written, released at publish or discard): a leased temp is never swept, whatever its age, so a slow transaction is not mistaken for a dead writer. The primary guard — a value shorter than `KB_CONVERSION_ARTIFACTS_TMP_MAX_AGE` is raised to it (reported once), so the effective lease always outlives the sweep threshold; a non-positive value is the default, with a warning. Needs a lock-capable cache store (Redis in production); a store that cannot lock leaves the age threshold alone in charge, reported once, never an ingest outage. |
| `KB_SOURCE_INFLIGHT_RESERVATION_SECONDS` | `kb.ocr.job_timeout` + 5 min (floor `600`) | How long an ingest **reserves** its source file for, so `kb:prune-orphan-files` never deletes a file a running ingest is still reading or converting. An ingest reads and converts its source — an OCR run takes minutes — BEFORE it takes the storage key's lock, so in that window the file has neither a row nor a holder and an ordinary-looking orphan may be a conversion's input; deleting it means the row then commits `full_copy` over an original that is already gone. The job takes the reservation before it reads a byte and drops it when it finishes, whatever the outcome, and the sweep names such a file on its own line (`kept (an ingest reserved it…)`) and counts it in `kept_meanwhile` — do not confuse that with the summary's `in_flight=`, which counts dangling OCR trees. The TTL is only the backstop for a worker killed mid-conversion, which is why it outlasts the longest conversion this deployment allows — a configured value shorter than that default is raised to it and reported once, because a reservation that expires mid-conversion is the guard silently switched off while appearing configured. `kb:ingest-folder --sync`, which converts inline, takes the same reservation, and the hard delete of a source honours it (the row still goes; the file waits for the sweep). Needs a lock-capable cache store (Redis in production); without one no reservation is taken and none is claimed, and the grace below stands alone. |
| `KB_ORPHAN_SOURCE_GRACE_SECONDS` | `3600` | Seconds a source file must be untouched before `kb:prune-orphan-files` may consider it an orphan — the **second** guard, for what the reservation cannot cover: a cache store that cannot lock, and a file written and queued whose ingest job has not started yet (plus a retry that could not re-take the reservation a killed worker left behind — reported in the log). A younger file is kept and counted `kept_meanwhile`. `0` disables the grace. On its own it never closed the window: an age threshold is a guess about how long work takes (OCR at the default `KB_OCR_JOB_TIMEOUT` of 3600 s can outlast the 3600 s grace), and it keys on when the BYTES were written rather than on when the conversion started, so a first ingest of a file staged earlier (a folder sync over a corpus copied days ago, a job that waited in a backed-up queue) gets no protection from it at all. |
...
| `KB_VERSIONS_ARTIFACT_STATE_CACHE` | `300` | Seconds a version's artifact state stays memoized for the timeline badges. The state is a READ of the stored bytes plus a SHA-256 check, so a page of `KB_VERSIONS_TIMELINE_LIMIT` versions would fetch that many objects from a bucket on every listing and every "load older" page; the memo is keyed by disk + path only — the memoized value is the `content_hash` verified at write time, checked against the row's current hash on every read, so a republished or repointed version (whose hash changed) misses and re-verifies rather than reading a stale entry, and a row with no hash to key against is never memoized. `0` verifies on every read (no lag, one object read per listed version). Only `verified` is memoized: a repairable state (`missing`, `mismatch`, `unverified`) is always re-read, so an identical re-ingest or a `kb:artifacts-backfill` run shows as repaired on the very next listing. What the window can hide is the one transition left — a verified file deleted or tampered inside it, whose badge may lag by up to the TTL — while `…/versions/{id}/content` and `…/versions/diff` re-read every time and report `missing` / `mismatch` faithfully. |
...
- **A "stored" badge is a verified claim, memoized.** `has_artifact` is derived
  from the same read + hash check the content endpoint uses and is true only for
  `verified` — memoized for `KB_VERSIONS_ARTIFACT_STATE_CACHE` seconds so a
  timeline page does not re-read one object per row on every listing (the key
  carries the bytes' identity; the content and diff endpoints never read the
  memo); `artifact_state` says `missing` or `mismatch` when the pointer no
