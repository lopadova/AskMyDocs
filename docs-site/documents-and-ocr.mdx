...
| `KB_CONVERSION_ARTIFACTS_SOURCE_LOCK_TTL` | `60` | Seconds that lock lives when its holder dies. |
...
- **The `markdown_only` drop is serialized with writers.** The reference scan and
  ...
  their row commit (only where a drop is possible: an artifact stored for a
  non-Markdown source — nothing waits on it with the flag off), and a sibling
  ...
  / `KB_CONVERSION_ARTIFACTS_SOURCE_LOCK_TTL` tune it (10 s / 60 s).
