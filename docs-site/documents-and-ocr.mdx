...
`reference_only`, the mode that promises no local copy at all: no run is
recorded, no figure is stored, the Markdown carries no `images/` reference
(ADR 0029 §5, ADR 0030 §3). Second, on a shared disk two tenants with the same source
...
"every PDF would need OCR" (R43). It
...
- **One core, both paths.** The write lives in `DocumentIngestor`'s shared
  ...
  never kept) and the loser drops its own temp.
...
- **Reads prefer the artifact and say so.** `contentFor()` returns the stored
  ...
  `reconstruction`, additive keys) so the UI labels a diff *faithful* only
  when both sides are stored documents, and an *index diff* otherwise. A
...
- **A tampered artifact is never served as faithful.** `content_hash` is the
  ...
  there was nothing to check against). On a local disk every read, delete and
  publish also resolves the real path and refuses one that a symlink under
  `.artifacts/` makes resolve outside the root.
- **`markdown_only` honours every row's contract.** Each row records the mode
  ...
  `reference_only` row never needed the local source.
...
- **Erasure covers it.** A hard delete removes the row's artifact; the prune
  ...
  `KB_CONVERSION_ARTIFACTS_TMP_MAX_AGE` plus artifacts no row (trashed rows
  ...
  across every tenant) and cover the configured `KB_DISK` / `KB_PATH_PREFIX`
  only. A soft delete leaves the artifact in place.
- **`markdown_only` keeps the sweeps honest.** The original is dropped only
  ...
  The backfill does not drop originals: it repairs history, it never changes it.
- **Backfill is operator-only.** `kb:artifacts-backfill` re-converts live
  rows without an artifact and writes one only when the reconversion hashes to
  the row's `document_hash` (`hash_mismatch` is reported and nothing is
  written — an artifact must agree with the version's chunks); `reference_only`
  rows are `intentionally_missing`, a missing source is reported, not invented.
...
| `KB_CONVERSION_ARTIFACTS_TMP_MAX_AGE` | `3600` | Seconds after which `kb:prune-archived-versions` sweeps a `.tmp` a dead writer left beside an artifact. |
| `KB_SOURCE_RETENTION` | `full_copy` | `full_copy` · `markdown_only` (drop the original binary after the artifact commit) · `reference_only` (no artifact, no `.ocr/` run). Wired for the first time in v8.36. |
...
- **Turning artifacts on populates nothing by itself.** Rows ingested before
  ...
  their next re-ingest or a `kb:artifacts-backfill --tenant=…` run.
