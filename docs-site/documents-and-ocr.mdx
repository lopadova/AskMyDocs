...
- **A "stored" badge is a verified claim.** `has_artifact` is derived from the
  same read + hash check the content endpoint uses; `artifact_state` says
  `missing` or `mismatch` when the pointer no longer resolves to the recorded
  bytes, and `kb:artifacts-backfill` repairs those rows.
