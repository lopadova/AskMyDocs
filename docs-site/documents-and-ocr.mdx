...
- A PDF the parser cannot read has no verified page count — the probe reports
  ...
  `tesseract` and `vision-llm` rasterise with `pdftoppm -l KB_OCR_MAX_PAGES`,
  so the file renders at most the cap whatever its object table claims, each
  page under the driver's timeout; `docling` hands the whole file to its
  engine and is refused the same way. Where it runs, the estimate marks the
  price as inexact.
