...
`{run}` is the full 64-hex SHA-256 of the bytes that were OCR'd, the driver and
its fingerprint — a truncated digest would be an identifier two different
inputs could share, and one run directory would then serve the wrong text.
