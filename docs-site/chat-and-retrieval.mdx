...
("documents from last month", "is this deadline past?", "what day is it?"). It is
explicitly marked usable even though it is not part of the retrieved Context, so
the [refusal contract](#the-refusal-contract) does not reject a plain "what time
is it?". The app runs on UTC; the timezone the bot *presents* is set by
`KB_PROMPT_TIMEZONE` (`config/kb.php` → `kb.prompt.timezone`, default
`Europe/Rome`) — a single render point shared by every LLM surface (chat,
conversation, streaming, agent, eval, benchmark).
