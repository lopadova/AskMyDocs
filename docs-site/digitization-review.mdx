...
external surface — HTTP and the CLI's default report mode — still refuses to
...
branch, so it applies uniformly:
...
non-canonical row has no `canonical_status` to penalize. Only the auto-tier
penalty — "is this generation_source `auto`?" — applies to both. Content that
never went through OCR (`generation_source` defaults to `human` everywhere
else, unaffected by this fix at all) sees zero change: the fix only changes
ranking for rows where `generation_source = 'auto'`, which since
...
| Read a page's status | `kb:review {document} --page={n}` (report mode) | `GET /api/admin/kb/documents/{id}/pages/{n}` | `KbReviewStatusTool` |
| Read the document summary | `kb:review {document} --report` | `GET /api/admin/kb/documents/{id}/review-summary` | `KbReviewStatusTool` |
...
- **The summary/page-status reads are ungated at the service layer, on
  purpose — but gated at the HTTP layer, also on purpose.**
  ...
  want the numbers regardless), but `KbReviewController` explicitly checks
  the flag before calling either one and returns a clean 404 when off. Don't
  assume "the service doesn't gate it" means "the HTTP surface doesn't
  either" — verify both, independently, the way
  `test_api_summary_returns_404_when_the_feature_is_disabled` does.
...
- **A document with no recorded `page_count` cannot be reviewed at all**,
  not even page 1 — if a document predates page-count tracking or was never
  converted through OCR/PDF processing, `setPageReviewStatus()` refuses it
  outright rather than accepting an unbounded page number.
