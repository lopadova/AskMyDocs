...
flowchart TD
    ...
    PROP --> CAND[(kb_text_correction_candidates\nstatus=pending, UNIQUE idempotency_key)]
    APC -->|"lockForUpdate, R21"| CAND
    REJ -->|"lockForUpdate, R21"| CAND
    APC -->|"old_text still matches CURRENT page"| DISK["Storage::put(source_path)"]
    DISK --> INGEST[IngestDocumentJob]
    APC --> AUDIT
...
A human reviewer lists the pending queue and approves it — this is the ONLY
action that touches the corpus, via `DocumentIngestor`:

```bash
curl /api/admin/kb/documents/42/corrections
...
# {"data": {"applied": true}}
# -> page 1's text is corrected on disk, IngestDocumentJob re-chunks +
#    re-embeds the WHOLE document (not just page 1 — a full re-ingest,
#    same as every other correction), a kb_canonical_audit row records it.

# Calling approve again on the same candidate is a safe no-op (R21):
curl -X POST /api/admin/kb/corrections/7/approve
...
- **A correction candidate's approval re-validates against the CURRENT
  version, not the one it was proposed against.** `proposeCorrection()`
  records `version_hash`, but `approveCorrection()` re-reads the document
  family's LIVE version and re-checks that `old_text` still occurs exactly
  once there. If a later edit changed or removed that text in the meantime,
  approval REJECTS the now-stale candidate (`stale_old_text_not_found_or_ambiguous`)
  rather than silently applying a correction to text that may no longer
  mean what the proposer thought — ADR 0031 §6's explicit requirement.
- **Approving a correction re-ingests the WHOLE document, not just the
  corrected page.** Same path `KbDocumentController::updateRaw()`'s manual
  edits take (CLAUDE.md §6's single ingestion execution path) —
  `EmbeddingCacheService` means pages whose text did not change cost no new
  provider call, but every chunk is re-derived.
