...
A canonical document declares a `canonical_type` in its frontmatter:

`project` · `module` · `decision` · `runbook` · `standard` · `incident` ·
`integration` · `domain-concept` · `rejected-approach`

…and a `canonical_status` lifecycle (`draft → accepted → deprecated →
superseded`, …). Canonical identifiers (`slug`, `doc_id`) are **tenant-scoped,
not global** — two projects can both own `dec-cache-v2`. The composite uniques are
`(project_key, slug)` and `(project_key, doc_id)`.

## The promotion pipeline (human-gated)

```mermaid
flowchart LR
    T[transcript / draft] -->|POST /promotion/suggest| SUG["LLM extracts candidates<br/>writes NOTHING"]
    SUG --> D[markdown draft]
    D -->|POST /promotion/candidates| VAL["CanonicalParser validates<br/>writes NOTHING"]
