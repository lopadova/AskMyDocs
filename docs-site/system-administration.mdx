...
|---|---|---|
| `system-admin` | Entire installation | Protected operator grant; owns `platform.admin` and `tenant.cross-access` |
...
flowchart TD
    ...
    R --> P["platform.admin"]
    R --> X["tenant.cross-access"]
    P --> UI["/app/system/tenants"]
    UI --> API["/api/system-admin/tenants"]
...
keeps the control plane usable when a tenant is suspended or archived.
