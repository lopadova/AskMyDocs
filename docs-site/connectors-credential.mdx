...
or worker failure marks the campaign failed and exposes **Resume full import**.
...
| `CONNECTOR_IMAP_BACKFILL_BATCH_SIZE` | `10` | Messages checkpointed by one import job for new campaigns. |
| `CONNECTOR_IMAP_BACKFILL_MAX_MESSAGES_PER_JOB` | `10` | Hard per-job cap, including campaigns persisted with a larger historical batch. |
| `CONNECTOR_IMAP_BACKFILL_FETCH_SIZE` | `5` | Messages fetched per IMAP bulk exchange inside a batch. |
| `CONNECTOR_IMAP_BACKFILL_STALE_MINUTES` | `20` | Reclaim a queued/running window after this heartbeat age. Keep it above the 75-second job timeout to avoid duplicate work. |
