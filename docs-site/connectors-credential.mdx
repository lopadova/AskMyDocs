...
a given `auth_mode`. `ConfigureConnectorService::splitPayload()` enforces a
**single secret per submission** (it throws on a second visible `target: 'secret'`
field), so it always sees exactly one secret and routes it to the vault, never
`config_json`.
...
app-only credentials are per-installation): `CONNECTOR_IMAP_GOOGLE_CLIENT_ID/_SECRET/_REDIRECT_URI`,
