...
flowchart TD
    subgraph FE[Admin SPA]
      TILE["Source tile (Available sources)<br/>auth_kind === 'credential'"] -->|"Add (+)"| MODAL["CredentialConnectorForm<br/>(rendered from credential_form_schema)"]
...
| `auth_mode` | Flow | Secret collected | User sign-in? |
|---|---|---|---|
| `basic` | password / app-password | `password` | no |
| `xoauth2` | **delegated** OAuth (Gmail / M365) | — (browser redirect + code exchange) | **yes** |
| `xoauth2_client_credentials` | **Microsoft 365 app-only** (client credentials) | `ms_client_secret` | **no** |

Provider env (delegated XOAUTH2 only — basic-auth and app-only need none, since
app-only credentials are per-installation): `CONNECTOR_IMAP_GOOGLE_CLIENT_ID`,
...
[connector-imap README](https://github.com/padosoft/askmydocs-connector-imap#microsoft-365--app-only-oauth2-client-credentials).
...
2. As a **super-admin**, open **`/app/admin/connectors`** → the **Email (IMAP)**
   tile in **Available sources** carries the **Add (+)** button.
3. Click **Add (+)** → a modal renders from the schema. For app-password auth:
...
4. **Connect** → the BE logs in (a real IMAP ping), vaults the password, and the
   account appears **Active** in the **Connections** list. Bad credentials → an
   inline error, no connection is created, nothing is vaulted.
...
See [Durable IMAP full-history imports](/connectors-imap-backfill) for the full
architecture, lifecycle, HTTP/MCP/PHP contracts, recovery model and production
runbook.

**Ingestion & Sync → Full mailbox import** does not place an entire mailbox in
one request or one queue payload. The backend snapshots each selected folder's
`UIDVALIDITY` and highest UID, divides its date range into durable windows, then
imports at most one bounded UID batch per job. A successful batch advances the
SQL checkpoint; completed windows are never scanned again. A terminal failure
marks the campaign failed and exposes **Retry full import**, which creates a new
snapshot rather than recycling an exhausted retry budget.
...
campaign. Starting requires an active IMAP installation.

| Environment variable | Default | Operational meaning |
|---|---:|---|
| `CONNECTOR_IMAP_BACKFILL_ENABLED` | `true` | Master switch. `false` keeps incremental sync available but makes status explicitly disabled and rejects starts. |
| `CONNECTOR_IMAP_BACKFILL_QUEUE` | `connectors` | Queue consumed by discovery, pump and bounded import jobs; run a dedicated worker for it. |
| `CONNECTOR_IMAP_BACKFILL_BATCH_SIZE` | `100` | Messages checkpointed by one import job. Higher values reduce queue churn but increase job duration. |
| `CONNECTOR_IMAP_BACKFILL_FETCH_SIZE` | `20` | Messages fetched per IMAP bulk exchange inside a batch. |
| `CONNECTOR_IMAP_BACKFILL_STALE_MINUTES` | `20` | Reclaim a queued/running window after this heartbeat age. Keep it above the 600-second job timeout to avoid duplicate work. |
| `CONNECTOR_IMAP_BACKFILL_ABSOLUTE_START` | `1970-01-01` | Inclusive oldest internal message date considered when constructing windows. Use an ISO `YYYY-MM-DD` date. |

Production needs both a `connectors` worker (or the configured queue name) and
