...
- **Stateful (SPA) sessions** — the request carries a session cookie; Sanctum
  ...
  authorization comes from the route's own gates — for the `/api/kb/*` group,
  `auth:sanctum` + `tenant.authorize` plus the controller/model AccessScope +
  project-membership scoping — not from token abilities).
...
- **Document viewer** → `GET /api/kb/documents/{documentId}/preview` returns a document's
...
- **The `token.ability` gate is a no-op for the SPA.** Do not assume it enforces
  ...
  own gates (`auth:sanctum` + `tenant.authorize` + AccessScope / project-membership
  scoping for the KB group). The gate only ever constrains real PATs.
