...
flowchart TD
    ...
    RT -->|X-Tenant-Id / user tenant / internal fallback| TC[TenantContext singleton]
    AUTH[auth:sanctum] --> ATH[AuthorizeTenantHeader<br/>resolved tenant requires membership]
    TC --> DEV[developer]
...
- **`ResolveTenant`** (middleware, runs early) sets the active tenant on the
  request-scoped **`TenantContext`** singleton — from the `X-Tenant-Id` header,
  the authenticated user's legacy `tenant_id`, or an internal reserved fallback
  that cannot authorize an operational route.
- **`AuthorizeTenantHeader`** (after `auth:sanctum`) closes the escalation hole:
  the resolved tenant always requires an explicit `project_memberships` row,
  even when the header is absent. Missing membership returns
  `403 tenant_forbidden`. Global `/api/auth/*` and `/api/system-admin/*` routes
  deliberately sit outside this middleware.
...
- Every tenant-aware table carries `string('tenant_id', 50)->default('default')->index()`.
  Tables introduced with the multi-tenant refactor also start their composite uniques
  with `tenant_id`; legacy v3 tables received the `tenant_id` column but their
  pre-existing composite uniques were not rebuilt (explicitly deferred in the
  `2026_04_28_000001_add_tenant_id_to_v3_tables.php` retrofit migration).
...
A request carrying `X-Tenant-Id: victim` is rejected
`403 tenant_forbidden` unless the identity has a membership in `victim`.
`system-admin` does not bypass this boundary; it reaches non-member tenants only
through the separate global control plane.
