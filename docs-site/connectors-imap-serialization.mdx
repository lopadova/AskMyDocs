...
production. `AppServiceProvider` only wires the decorator when
`serialize_connections` is on **and** the active cache store implements
`LockProvider` with real atomic locks. On a non-lock-capable dev store (file /
array) it **degrades to a process-local lock** — still correct within a single
process — and on a store that cannot lock at all it disables serialization
