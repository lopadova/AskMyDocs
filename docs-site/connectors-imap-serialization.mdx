...
its retry budget. The middleware key is explicitly `shared()` with IMAP backfill
discovery and window-import jobs: Laravel otherwise scopes an overlap key by job
class, which would let incremental sync and full-history import collide. Admin
"sync now" dispatches the same serialized job.

## Configuration

The knobs below live in `config/connectors.php`, env-overridable. The **Config key**
column is relative to `connectors.imap` **except where the row marks it top-level**
(`fake_imap_ping` sits at `connectors.fake_imap_ping`, NOT `connectors.imap.*`):
...
| `CONNECTOR_IMAP_FAKE_PING` | `fake_imap_ping` *(top-level)* | `false` | Offline/E2E seam. When on, serialization stands down (no real server to protect); the serialized job degrades to the vendor job. |
...
If the store cannot provide locks at all, the app skips serialization without breaking functionality. This is the [R43](/isolation-testing)
