...
| Agent investigation depth ("livello di approfondimento") | 1-5 per-run knob, chat composer's `chat-depth-picker` select (`frontend/src/features/chat/Composer.tsx`), lifted in `use-chat-session.ts` and threaded through `useAgentChat` → `chatApi.startAgentTurn()`. `AgentMessageController` validates it (`between:1,5`, default `agent.depth.default`=3) and stamps it onto `AgentRun.input_json.depth`. `App\Agent\Budget\AgentBudgetTracker::limit()` applies a config-driven multiplier (`agent.depth.multipliers`, `AGENT_DEPTH_MULTIPLIER_1..5`) to the depth-eligible entries in `agent.limits.*` — iterations, logical_soft/hard, physical_hard, both time budgets, evidence_bytes — giving `AgentLoop`'s existing plan→act→observe→re-plan loop more (or fewer) cycles to decide it needs another cascading KB search. Loop-safety guards (`consecutive_errors`, `duplicate_calls`) are NEVER scaled. Level 3 = 1.0x = the unscaled pre-existing defaults, so an omitted/default depth is byte-identical to before this knob existed. |
