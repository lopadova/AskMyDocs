...
| AI FinOps (v8.14, `padosoft/laravel-ai-finops` + `-admin`) | `app/FinOps/AiCallMeter.php` (full-coverage metering on AiManager), `app/FinOps/HostTenantResolver.php`, `app/Http/Middleware/FinOpsAuthorize.php` (method-aware gate), `config/ai-finops.php` + `config/ai-finops-admin.php` (secure host overrides, R32), gates `viewAiFinOps`/`manageAiFinOps` in `AppServiceProvider` |
