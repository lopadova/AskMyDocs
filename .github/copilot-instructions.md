...
| Sessions workspace (v8.x) | `frontend/src/features/chat/use-chat-session.ts` (the SHARED turn orchestration both `/chat` and `/sessions` consume — never fork it), `frontend/src/features/chat/sessions/*`, `app/Services/Chat/{ConversationOrganizerService,ChatFolderService}.php`, `app/Support/Chat/ConversationImportance.php`, `app/Models/ChatFolder.php`, `app/Http/Resources/Chat/ConversationResource.php` (`$wrap = null` — the list is a BARE array, R27). Organisation writes use `timestamps = false`; folder delete is `nullOnDelete`. |
| Reader KB browse (v8.x) | `app/Http/Controllers/Api/KbTreeReaderController.php` (`GET /api/kb/tree`, same `KbTreeService` core, `with_trashed` never forwarded), `frontend/src/features/kb-browse/*`. |
