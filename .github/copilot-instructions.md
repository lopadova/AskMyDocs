...
| System administration | `app/Support/PlatformAccess.php`, `app/Services/Admin/{SystemAdminAccessService,SystemAdminTenantService,TenantProvisioningService}.php`, `app/Http/Controllers/Api/SystemAdmin/TenantControlController.php`; `system-admin` owns `platform.admin` + `tenant.cross-access`, while tenant `super-admin` still requires membership. Global role grant/revoke is audited CLI-only (`system-admin:grant|revoke --yes`); `/api/system-admin/*` is capability-gated and tenant-header-exempt; no MCP write surface by ADR 0023. |
