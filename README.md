...
| Filesystem Explorer (grid-browse + preview + bulk) | A `view=explorer` toggle on **Admin → Knowledge Base** (`/app/admin/kb`) switches the tree+detail layout to a media-gallery-style explorer: virtual folders (derived from `source_path` segments) browsed as breadcrumb + folder tiles + document cards, grid/list toggle with adjustable tile size, a markdown preview aside that reuses the detail Preview renderer, and file-manager multi-select (click / ctrl-cmd / shift / select-all). Bulk actions: ZIP-export the selected markdown (`GET /api/admin/kb/documents/zip` — folder structure preserved + a `manifest.json` of included/skipped files, never an empty zip under 200, R14), soft/force delete (`POST .../bulk-delete`) and restore (`POST .../bulk-restore`) — all tenant-scoped + `withTrashed`-aware through the single `DocumentDeleter` path; reads the whole tree from the existing `useKbTree` cache (zero new fetches) | v8.8 |
