...
| **v7.0/W5** | ⏳ planned | Host integration over the FULL v1.3 package — single AskMyDocs rewrite replacing v5.0's inline `app/Mcp/Client/*` with the package, picking up SSE + resources/prompts + circuit-breaker telemetry + server-side capabilities all in one pass. The pre-staged `feature/v7.0/W1.B-host-integration` branch (host bridge + adapters + custom sidecar transport + audit migration) gets rebased and extended; one Final PR closes v7.0 |
