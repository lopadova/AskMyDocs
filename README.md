...
v1.0 through v8.1.0. **v8.1.0** is a retrieval-quality minor release:
it fixes a production-broken anti-hallucination refusal gate, unifies
the three chat channels onto one retrieval + citation path, turns
`@mention` into a recall-safe rerank boost, ships evidence-grade
citations + result diversification, and adds an IR-metrics core
(nDCG@k / MRR / precision@k). v8.0.3 (the prior release) closed a
26-finding multi-tenant isolation + security review.
