...
<p align="center">
  ...
  <a href="CHANGELOG.md"><img src="https://img.shields.io/badge/release-v6.0.0-blueviolet?style=flat-square" alt="Release v6.0.0"></a>
  ...
  <a href="#quality--observability"><img src="https://img.shields.io/badge/tests-1695%20PHPUnit%20%2B%20408%20Vitest-brightgreen?style=flat-square" alt="1695 PHPUnit + 408 Vitest"></a>
</p>
...
| **v7.0/W2** | ⏳ planned | mcp-pack v1.1.0 — SSE transport (`SseJsonRpcTransport`) for remote HTTP+SSE gateways, JSON-RPC `resources/*` + `prompts/*` methods so the orchestrator can read from upstream resource catalogs and pre-prompt templates |
| **v7.0/W3** | ⏳ planned | mcp-pack v1.2.0 — first-class server-side. The same package exposes a Laravel app AS an MCP server (stdio long-lived process via artisan command + HTTP+SSE route + JSON-RPC handler routing initialize / tools/list / tools/call to host-supplied tool catalog). Auth + RBAC integration with host gates |
| **v7.0/W4** | ⏳ planned | mcp-pack v1.3.0 — production-hardening. Per-tool circuit breaker (open / half-open / closed states tracked in cache with TTL recovery) + adaptive retry budget (token-bucket per server per minute, exponential backoff on failure). Decorator over `ToolInvoker`; new config keys + telemetry events |
| **v7.0/W5** | ⏳ planned | mcp-pack v1.4.0 — admin backend surface. Package registers REST routes under a configurable prefix (default `/api/admin/mcp-pack-pack`): server CRUD, handshake action, tool catalog, paginated audit log, circuit-breaker state. Middleware-driven auth (host wires Sanctum / RBAC). OpenAPI 3.1 spec + Postman collection ship with the package. NO React/Vue code — this is the backend the standalone `-admin` SPA will consume in the post-v7.0 cycle |
| **v7.0/W6** | ⏳ planned | Host integration over the FULL v1.4 package — single AskMyDocs rewrite replacing v5.0's inline `app/Mcp/Client/*` with the package, picking up SSE + resources/prompts + circuit-breaker telemetry + server-side capabilities + admin REST routes all in one pass. The existing inline `McpServersAdminController` + `McpToolCallAuditController` get retired; the host's React MCP admin screens stay, just point at the package's new REST endpoints. The pre-staged `feature/v7.0/W1.B-host-integration` branch (host bridge + adapters + custom sidecar transport + audit migration) gets rebased and extended; one Final PR closes v7.0. The companion `padosoft/askmydocs-mcp-pack-admin` standalone React SPA ships as a follow-up cycle AFTER v7.0 — it consumes the same v1.4 routes and can be cross-mounted then |
...
| `padosoft/askmydocs-mcp-pack-admin` | Standalone React SPA companion — server CRUD + handshake + tool catalog + paginated audit log + circuit-breaker dashboard, consuming the v1.4 REST routes the main package exposes. Cross-mounts under `/admin/mcp-pack` exactly like `pii-redactor-admin` / `flow-admin` / `eval-harness-ui` | 📅 planned post-v7.0 — separate repo + own R36 cycle once v1.4 backend ships and v7.0/W6 host integration is green | n/a |
