...
- **Agent-proposed text corrections (ADR 0031 §6-7)**: an agent may `propose` a probable transcription fix via the MCP `KbProposeTextCorrectionTool`, but only ever writes a CANDIDATE row (`kb_text_correction_candidates`) — never the corpus itself, per ADR 0003's "agent proposes, a person commits" boundary. `old_text` must occur exactly once on the candidate's page; a replayed call (identical tenant/actor/document/version/page/old/new) returns the SAME candidate rather than a duplicate, and every outcome (accepted/denied/replayed) is audited. A human reviewer then `approve`s or `reject`s the candidate over HTTP only (no MCP write for that decision) — approval re-validates `old_text` against the family's CURRENT live version (rejecting a now-stale candidate rather than silently misapplying it) and, on success, mints a genuinely NEW version through `DocumentIngestor::reembedFromMarkdown()` — never a write to the original `source_path` binary — with the prior version archived and a `kb_canonical_audit` row recording it.
