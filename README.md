...
| **v8.36 → v8.40** | ⏳ designed 2026-09-11 | **Document intelligence + portable LLM Wiki export.** A competitor audit ([Annota AI](docs/v4-platform/AUDIT-2026-09-11-annota-ai-gap.md)) found the one seam where a data-preparation platform is ahead of us: *file → reviewed Markdown*. We have **no OCR** — `PdfConverter` reads the text layer only and `SourceType` accepts no `image/*`, so a scanned contract arriving through IMAP or OneDrive is a 422 or an empty document — and the converted Markdown is **not stored** (`source_retention`/`markdown_path` are still the ADR 0014 schema foundation), so nothing can be reviewed or exported, and the v8.7 *Cloud Time Machine* diffs chunk reconstructions rather than documents. Five workstreams, each behind a default-OFF flag (R43): **W1** `OcrConverter` with pluggable drivers (docling / Mistral OCR / vision-LLM / tesseract), figures to `images/`, LaTeX formulas, per-page confidence, extraction origin `ocr` on document and chunks (orthogonal to the ADR 0028 `provenance_tier`) + PII redaction before embedding + FinOps metering with a cost estimate on the upload modal; **W2** stored conversion artifacts on the existing *Cloud Time Machine* (v8.7) — faithful diff/restore of the document instead of a chunk reconstruction, versions born on correction with `actor`/`reason`, and the MCP read surface the feature never got; **W3** *Digitization Review* — side-by-side original vs Markdown with a confidence heat-map, non-canonical OCR documents born in the `auto` tier (canonical frontmatter keeps its own say) and promoted to `human` on approval (ADR 0014 already models it), an MCP `KbProposeTextCorrectionTool` that **proposes** and a human-only review status (the explicit inverse of Annota's `update_document_page`), CER/WER metrics as host-side classes registered with `eval-harness` (no package change); **W4** `kb:export-wiki` — the Karpathy layout (`raw/`, `wiki/`, `index.md`, `log.md`, `AGENTS.md`, `CLAUDE.md`) but **already compiled**, tiered (`tier`/`evidence`/`provenance_tier`/`extraction` frontmatter, rejected-approaches included), ACL-filtered (R33), hash-manifested, with a `.mcp.json` back to the live server, and `kb:import-wiki` returning edits as promotion candidates; **W5** Auto-Wiki maintenance as a `laravel-routines` routine with a mandate and a pause-and-ask (explicit new dependency, cron path unchanged). Absorbs the *source-retention wiring* and *content export/portability* items below. [PLAN](docs/v4-platform/PLAN-v8.36-document-intelligence-and-llm-wiki-export.md). |
