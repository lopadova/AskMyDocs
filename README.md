...
**Sister packages composer constraints (v4 release train)** — `padosoft/laravel-ai-regolo:^1.0`, `padosoft/laravel-pii-redactor:^1.2`, and `padosoft/laravel-flow:^1.0` are all in `require` (load-bearing for the chat path, v4.1 PII redactor integration, and v4.2 ingest-pipeline orchestration respectively); `padosoft/eval-harness` remains in `require-dev` (CI-only, scoped for v4.2 sub-PR 4). `padosoft/laravel-patent-box-tracker` is intentionally NOT declared in AskMyDocs's `composer.json` — operators install it in their own Laravel project per R37 (see [Patent Box dossier](#patent-box-dossier-v40-dogfood)).
```json
{
    "require": {
