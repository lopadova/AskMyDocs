...
| AI deep-analysis on document change (Doc Insights) | When a document is **ingested or modified**, an async `AnalyzeDocumentChangeJob` asks the LLM — given the changed doc + its closest semantic neighbours — to (a) suggest how to strengthen it, (b) surface its cross-references, and (c) flag which OTHER docs the change makes obsolete / in need of revision (each with a suggested action). Results land in `kb_doc_analyses`, notify the doc's reviewers (`kb_doc_analysis_ready`), and render under **Admin → Doc Insights** (`/api/admin/kb/analyses`). **Suggest-only** — never mutates a doc (ADR 0003). Cost-gated: default ON for canonical docs (`KB_CHANGE_ANALYSIS_CANONICAL`), opt-in for non-canonical (`KB_CHANGE_ANALYSIS_NON_CANONICAL`); debounced; master switch `KB_CHANGE_ANALYSIS_ENABLED` | v8.7 |
...
v1.0 through v8.6.0. **v8.6.0** makes the chat's dead clickable actions live:
