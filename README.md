...
- **API connector — endpoints as live LLM tools** (v8.27, `padosoft/askmydocs-connector-api`) — a
  ...
  only** — the model supplies just the `llm` parameter values and never sees URLs/headers/secrets.
   ...
   Gemini. Configure under **Admin → API Connectors**. The durable multi-call retrieval loop, SSE protocol, safety budgets and mock-server test procedure are documented in the [agentic API retrieval runbook](docs/runbooks/agentic-api-retrieval.md). **Full-history imports are durable and bounded**: selected folders are divided into date windows, each UID batch checkpoints in SQL, and `kb-ingest` workers index independently. Configure the master switch, queue, batch/job caps, fetch size, stale-worker recovery and oldest accepted date with `CONNECTOR_IMAP_BACKFILL_ENABLED`, `CONNECTOR_IMAP_BACKFILL_QUEUE`, `CONNECTOR_IMAP_BACKFILL_BATCH_SIZE`, `CONNECTOR_IMAP_BACKFILL_MAX_MESSAGES_PER_JOB`, `CONNECTOR_IMAP_BACKFILL_FETCH_SIZE`, `CONNECTOR_IMAP_BACKFILL_STALE_MINUTES` and `CONNECTOR_IMAP_BACKFILL_ABSOLUTE_START`.  The operation is available through Admin HTTP/UI and the write-capable `KbImapBackfillTool` MCP tool.
...
With `KB_OCR_ENABLED=true` (v8.36, ADR 0029) the walker, the API, the upload modal and the connectors also accept `.png`, `.jpg`/`.jpeg`, `.tif`/`.tiff` and `.webp` (`image/*` → `SourceType::IMAGE`), and a PDF with no text layer is routed to OCR instead of being ingested empty. With the flag off (the default) every entry point refuses images exactly as before — see [Scanned documents & OCR](#scanned-documents--ocr-v836) below.
...
- `OcrConverter` (v8.36, `KB_OCR_ENABLED=true` only) — `image/png`, `image/jpeg`, `image/tiff`, `image/webp`; runs the configured OCR driver and emits `## Page N` sections with per-page confidence and figures (see [Scanned documents & OCR](#scanned-documents--ocr-v836))
...
**PDF support:** `smalot/pdfparser` is a hard `require` (pure PHP, no system deps). For more robust extraction on complex PDFs (multi-column layouts, certain XFA forms, mixed encodings), install `poppler-utils` on the host (`apt install poppler-utils` on Debian/Ubuntu, `brew install poppler` on macOS) — the `PdfConverter` automatically falls back to the `pdftotext` binary when smalot raises an exception (`KB_PDFTOTEXT_BIN` names the binary; with OCR enabled, a PDF that neither smalot nor pdftotext can read text from is routed to OCR — a parser failure alone never is). `extractionMeta.extraction_strategy` records which strategy was used per document so you can audit the rate of fallbacks in production.

### Scanned documents & OCR (v8.36)

Default **OFF** (`KB_OCR_ENABLED=false`, [ADR 0029](docs/adr/0029-v836-ocr-converter-drivers-and-ocr-provenance.md)). When on:

- **Images** (`png`/`jpeg`/`tiff`/`webp`) become documents through `OcrConverter`; **scanned PDFs** stay with `PdfConverter`, which probes the text layer **per page** (`KB_OCR_PROBE_MIN_CHARS` per page over every page up to `KB_OCR_MAX_PAGES`; `KB_OCR_PROBE_PAGES` bounds the window) and delegates to `OcrService` when no page has text (`empty`), when typed pages and scanned pages coexist (`mixed` — the whole document is OCR'd so no page is lost) or when nothing can read the file. `PdfConverter` is the only PDF match — no overlapping `supports()` (R23).
- **Drivers** (`KB_OCR_DRIVER`): `tesseract` (default, local CLI), `docling` (local CLI, best layout/tables), `mistral-ocr` (remote API), `vision-llm` (a `laravel/ai` vision call on the configured chat provider), `fake` (tests only, refused in production). Remote drivers send the document bytes to a third party **before** the PII seam can see the text, so they run only with `KB_OCR_ALLOW_REMOTE=true` — the registry fails closed otherwise; `mistral-ocr` posts only to a host in `KB_OCR_MISTRAL_ALLOWED_HOSTS`, `vision-llm` goes through `AiManager` like every chat call, and every run is capped **before** egress by `KB_OCR_MAX_PAGES` / `KB_OCR_MAX_BYTES`.
- **Provenance**: every chunk carries `metadata.provenance = "ocr"` and `ocr_confidence`; the document records `extractionMeta.ocr.{driver, remote, reason, pages[], mean_confidence, figures}`. This is the *extraction origin*, orthogonal to the ADR 0028 `provenance_tier` (source authorship).
- **PII**: `ChunkRedactor` (ADR 0020) runs unchanged on the OCR output before embedding. **Figures** land at `{source}.ocr/{run}/images/fig-{page}-{n}.png` on the KB disk (`{run}` = content-addressed key of the OCR'd bytes **and** the engine — driver, its fingerprint, the figure switch — so versions and engines never overwrite each other, and an identical re-ingest through the same engine reuses the recorded run instead of paying again; `KB_OCR_REUSE_ENABLED=false` turns the recorded run off and makes every ingest a new run with its own directory — figures follow `KB_OCR_FIGURES_ENABLED` and the retention mode) and are referenced as `![Figure p.n](images/fig-p-n.png)`.
- **FinOps**: each run is a metered `ocr` purpose (`KB_OCR_RATE_PER_PAGE` × pages for locally-priced drivers; `vision-llm` is metered per token by the SDK hook, not double-counted). The upload modal shows the **estimate before commit** (`GET /api/admin/kb/uploads/{batch}/estimate`).
- **Tri-surface** (R44): `kb:ocr {document} [--status]` · `GET|POST /api/admin/kb/documents/{id}/ocr` · MCP `KbOcrStatusTool` (read-only — an agent can see, not spend).
- **OFF path** (R43): images are refused with the same 422 / "Unsupported file type" as before at the API, the upload modal, the folder walker **and** the connector bridge (recorded as `connector_ingest_refused` in `kb_canonical_audit`); scanned PDFs ingest as empty documents exactly as today.

Docs: [Documents & OCR](https://padosoft.mintlify.app/documents-and-ocr).

### Digitization Review (v8.37)
...
- **Agent-proposed text corrections (ADR 0031 §6-7)**: an agent may `propose` a probable transcription fix via the MCP `KbProposeTextCorrectionTool`, but only ever writes a CANDIDATE row (`kb_text_correction_candidates`) — never the corpus itself, per ADR 0003's "agent proposes, a person commits" boundary. MCP calls carry a tenant-scoped token with no per-user identity, so proposals share a fixed `mcp:kb-propose-text-correction` actor per tenant, honestly, rather than a fictitious per-user one. `old_text` must occur exactly once on the candidate's page; a replayed call (identical tenant/actor/document/version/page/old/new) returns the SAME candidate rather than a duplicate, and every outcome — accepted, EVERY denied reason, and replayed — is audited, with the whole check-then-act sequence serialized per tenant+actor. A human reviewer then `approve`s or `reject`s the candidate over HTTP only (no MCP write for that decision) — approval is a three-phase flow that atomically claims the candidate (`applying`) before applying anything, refuses a CANONICAL document outright (its YAML frontmatter can't survive chunk-reconstructed re-embedding), re-validates `old_text` against the family's CURRENT live version — reconstructed with page boundaries re-synthesized from chunk metadata when no artifact is retained — (rejecting a now-stale candidate rather than silently misapplying it), and, on success, mints a genuinely NEW version through `DocumentIngestor::reembedFromMarkdown()` — never a write to the original `source_path` binary, and never inside an ambient transaction that could orphan its side effects — flips the candidate to the terminal `applied` state, and writes a `kb_canonical_audit` row (best-effort — a write failure there is logged, never reported to the caller as approval failure). A candidate stuck `applying` after a crashed approval is recoverable via `kb:review-reconcile-stuck-corrections`, not silently indistinguishable from success.
- **Tri-surface, so far**: `kb:review {document} [--page=N --status=reviewed|unreviewed] [--approve] [--report]` (CLI) · `GET /api/admin/kb/documents/{id}/review-summary` (document-wide), `GET .../pages/{n}` (single page), `PATCH .../pages/{n}/review-status`, `POST .../approve`, `GET .../documents/{id}/corrections` (paginated pending queue), `POST .../corrections/{id}/approve`, `POST .../corrections/{id}/reject` (HTTP, admin-gated) · MCP `KbReviewStatusTool` (read-only — reports progress, never marks or approves anything itself) + `KbProposeTextCorrectionTool` (write — a CANDIDATE only). `kb:review-reconcile-stuck-corrections` is a documented R44 single-surface exception (CLI-only, no caller-facing read) — see [Crash recovery](https://padosoft.mintlify.app/digitization-review#gotchas).
...
- **Not yet shipped** (deferred past v8.37 GA, documented in the [closure STATUS](docs/v4-platform/STATUS-2026-09-20-v837-w3-closure.md)): the *advanced* review UI (original ↔ Markdown side-by-side, confidence heat-map), the CER/WER quality metrics, and the `laravel-flow` review-queue workflow integration.
- **Config**: `KB_REVIEW_LOW_CONFIDENCE_THRESHOLD=0.70` — the OCR confidence (chunk metadata, W1) below which a span is highlighted in the not-yet-shipped review UI's heat-map; a visual aid only, it never gates approval. `KB_REVIEW_CANDIDATES_PER_HOUR=60` — the rate cap on `KbProposeTextCorrectionTool`, shared **per tenant**, not per user (ADR 0031 §6): the MCP connection carries a tenant-scoped token, not a per-user one, so every caller in the tenant spends from the same hourly budget. `KB_REVIEW_CORRECTIONS_PAGE_SIZE=50` — the cap on `GET .../documents/{id}/corrections`'s pending-candidate queue.

Docs: [Digitization Review](https://padosoft.mintlify.app/digitization-review).

### Portable Wiki Export + candidate-only import (v8.38)

Default **OFF** (`KB_WIKI_EXPORT_ENABLED=false`, [ADR 0032](docs/adr/0032-v838-portable-wiki-export-and-candidate-only-import.md)) — the one flag gates every export AND import surface; there is no separate import-specific flag. W4a shipped the synchronous CLI export core: `kb:export-wiki {--tenant} {--project} {--as-user} {--output}` builds an ACL- and PII-scoped folder (`wiki/`, `raw/`, `README.md`, `AGENTS.md`, `CLAUDE.md`, `MANIFEST.json`) under an operator-chosen directory. The exported set is exactly what `--as-user` may retrieve — computed by authenticating as that user for the export's duration so `AccessScopeScope` (the same global scope every other read surface goes through) applies to the document query itself, never filtered after the fact. `raw/` is the stored conversion artifact or an explicit `raw_missing` manifest entry (never a chunk reconstruction passed off as the original); `wiki/` is the readable compiled page and may legitimately fall back to a reconstruction. Both `raw/` and `wiki/` go through the tenant's PII policy when active — the export inherits the same redaction contract as ingestion, not a weaker one. `MANIFEST.json` hashes every file (sha256) plus a running chain hash so a folder found later can be checked for internal consistency (accidental corruption, a truncated copy) — the chain hash is unkeyed and stored in the same folder it covers, so it is a corruption check, not cryptographic tamper evidence against a malicious actor; that would need a server-held, out-of-band signature, which is not designed yet.

**W4b adds the async HTTP surface** (ADR 0032 §5/§11/§12): `POST /api/admin/kb/exports` (`role:admin|super-admin`) computes an idempotency key from the tenant/principal/project/options/corpus snapshot/authorization digest — a repeat request while nothing has changed reuses the same bundle instead of re-exporting — and queues `ExecuteKbWikiExportJob`, which reloads the requesting `User` by id (never a serialized `Auth` object), re-checks they still hold the export permission, authenticates as them for the export's duration, and clears the guard in `finally` (mirrors `ExecuteAgentRunJob`'s own principal-restoration discipline so a reused queue worker can never leak one export's principal into the next). The job zips the folder and stages it on `kb.staging.disk` (ADR 0029's disk, reused — not a new one). `GET /api/admin/kb/exports/{id}` polls status; `GET .../{id}/download` re-authorizes the CURRENT session against the export's recorded document ids and answers `403 export_invalidated` if any are no longer visible, rather than serving bytes a stale idempotency key happened not to catch — the second, independent gate the ADR requires alongside the idempotency key itself. `kb:prune-wiki-exports [--dry-run]` sweeps expired bundles by `expires_at`, scheduled hourly (`onOneServer()->withoutOverlapping()`, distinct from the once-nightly `kb:prune-staging-batches`).

**W4c closes the loop** (ADR 0032 §4/§7/§11): every exported folder now ships a `.mcp.json` at its root (`KbWikiExportService::buildMcpConfig()`) pointing at the tenant's live MCP server, with `${ASKMYDOCS_MCP_TOKEN}`/`${ASKMYDOCS_TENANT_ID}` as placeholder environment-variable references — never an embedded credential. Every exported page's frontmatter now round-trips through `CanonicalParser` (`slug`/`id`/`type`/`status` ahead of the existing governance fields), so a page pulled back — by hand, by an agent, or via `kb:import-wiki {folder} --tenant= --as-user=` walking a whole local `wiki/` tree — parses back into the same canonical identity it was exported with. Import is **candidate-only** (ADR 0003's "agent proposes, a person commits" boundary, same shape as v8.37/W3b's correction candidates): `KbWikiImportService::importDocument()` never writes `knowledge_documents` itself, only dispatches the same `PromotionFlow` saga `KbPromotionController::promote()` already uses, pausing at a human approval gate. A replay of an already-proposed change (`kb_wiki_import_candidates`'s `idempotency_key`) returns the existing candidate rather than a fresh approval token — `ApprovalTokenManager::reissuePendingForStep()` is one-shot by design. An edit that only changes frontmatter (`type`/`status`/`retrieval_priority`) with an unchanged body still proposes a candidate; a body-only diff would have silently dropped it. Tri-surface (R44): the HTTP `POST /api/admin/kb/imports` and MCP `KbImportWikiTool` both propose a single document; only the CLI walks a local folder, since local filesystem access is the point (same rationale as export's CLI-only shape). `KbCreateExportTool`/`KbGetExportTool` complete the export side's MCP surface.
- **Not yet shipped**: `llms.txt`/`llms-full.txt`, and `include_images`/`--format` variants (both explicitly rejected, never silently ignored, by the async request service — only `format="llm-wiki"` and `include_images=false` are implemented).
- **Config**: `KB_WIKI_EXPORT_ENABLED` (the gate for every export AND import surface), `KB_WIKI_EXPORT_RETENTION_HOURS=24` (drives both the DB row's `expires_at` and the hourly sweep), `KB_WIKI_EXPORT_QUEUE=default` (queue name for `ExecuteKbWikiExportJob`), `KB_WIKI_IMPORT_CANDIDATES_PER_HOUR=30` (per-tenant import-proposal rate limit, checked after the idempotency lookup so a replay never spends the budget), `KB_WIKI_EXPORT_CREATE_REQUESTS_PER_HOUR=10` (per-principal rate limit on MCP-initiated export requests).

Docs: [Portable Wiki Export](https://padosoft.mintlify.app/portable-wiki-export).

### Auto-Wiki maintenance as a delegated routine (v8.39)

Default **OFF** (`KB_WIKI_ROUTINE_ENABLED=false`, [ADR 0033](docs/adr/0033-v839-auto-wiki-maintenance-as-a-delegated-routine.md)) — `kb:wiki-maintain`'s existing nightly cron (index rebuild + lint + backfill un-enriched docs) can optionally run as a named, observable **routine** instead of an anonymous scheduler entry, via `padosoft/laravel-routines` v1.2.0 — a real agentic-routine engine (schedule, dispatch, mandate/consent, budget, escalation), a `require` Composer dependency like every other `padosoft/*` package in this repo (ADR 0033 corrects the plan's original "optional dependency" framing once the real package was installed and read: "optional" means the *feature* an operator may not enable, never the *package* an installation may not have). `WikiMaintenanceRoutineTarget implements RoutineTarget` — a thin adapter over the UNCHANGED `WikiMaintainer` core, declaring the same two action classes (`kb.wiki.compile`, `kb.wiki.lint`) it has always had. `WikiRoutineService` is the one core behind the tri-surface — status, run, and auto-provisioning a `Routine` row on first use, the package's generic `organization_id` column repurposed to hold this app's `tenant_id` (the engine has no tenant concept at all; R30 scoping is entirely this service's job, not the package's). `WikiMaintenanceRoutineGate` is the "no double run, no orphaned run" scheduler decision — keep the legacy cron slot (the safe default) unless the flag is on, the adapter is registered, AND an active routine exists **for the tenant the legacy slot itself services** (`default` — `KbWikiMaintainCommand`'s own signature default), wrapped in `try/catch` and failing safe to "keep the cron" on any database error, since the check runs on every console boot, not only `schedule:run`. The package's own generic admin API (`api/routines/v1`) and its signed webhook ingress (`hooks/routines/{id}`) are both turned **off** by default (`ROUTINES_API_ENABLED=false`, `ROUTINES_WEBHOOKS_ENABLED=false`) — the admin API has no tenant boundary and defaults to cross-tenant read access for any authenticated user with no policy defined, and nothing this app creates is a `trigger_kind=webhook` routine. Tri-surface (R44): `kb:wiki-routine {status|run}` · `GET /api/admin/kb/wiki-routine` + `POST .../run` (`role:admin|super-admin`) · MCP `KbWikiRoutineStatusTool` (read-only — no MCP `run` tool, documented R44 exception, same posture as W1's OCR re-run and W3's review approval).
- **Documented gap, not an oversight** (ADR 0033 §7): no mandate/consent-grant step-up flow is wired this cycle. The routine runs with exactly the authority the unattended cron already had; the engine's `MandateExceeded`/pause-and-ask machinery is real and wired through, but nothing this cycle can trigger it.
- **Independent review caught two must-fix issues before merge** (R36 fallback — neither Copilot nor `@codex review` responded): the gate's "an active routine exists" check was originally un-scoped by tenant, so any OTHER tenant enabling the routine for itself would have silently disabled the shared cron slot `default` still depends on; and the routine-provisioning find-then-create had no lock, a TOCTOU race that could double-provision a routine under concurrent triggers. Both fixed with regression tests proving the actual invariant, not just sequential idempotency.

Docs: [Auto-Wiki](https://padosoft.mintlify.app/auto-wiki).
...
| Image labelling · Magic Select/Annotate · COCO · YOLO training | ❌ | ❌ — **out of domain by design** (audit §3: a different market); the one adjacency worth a small move, a `vision` column in Tabular Review, shipped as **v8.40.0** (W6, [ADR 0034](docs/adr/0034-v840-vision-column-in-tabular-review.md)) once a real image-heavy tenant signal arrived — colour/material/pattern extraction over OCR figures or a standalone product photo, human-reviewed in the existing grid | ✅ / 🔜 |
...
| Scanned documents & OCR (default OFF) | `OcrConverter` for `image/*` + text-layer probe fallback inside `PdfConverter`; pluggable drivers (`tesseract` / `docling` / `mistral-ocr` / `vision-llm`, remote ones behind `KB_OCR_ALLOW_REMOTE`); figures to `{source}.ocr/{run}/images/`; `provenance: ocr` + `ocr_confidence` on every chunk; PII redaction before embedding; FinOps `ocr` metering + pre-commit estimate on the upload modal; tri-surface `kb:ocr` / `…/documents/{id}/ocr` / `KbOcrStatusTool` | v8.36 |
...
| Idempotent SHA-256 ingestion | Composite UNIQUE on `(tenant_id, project_key, source_path, version_hash)`; re-pushing identical bytes is a no-op; a new version archives the prior; `$tries=3` with backoff `[10, 30, 60]` on `IngestDocumentJob` | v1.0 |
...
| Conversion artifacts on the Time Machine (ADR 0030) | The v8.7 Time Machine diffed chunk *reconstructions*; with `KB_CONVERSION_ARTIFACTS_ENABLED=true` (default OFF, R43) every version stores the exact Markdown the chunker received at `.artifacts/{tenant}/{project}/{source_path}.versions/{version_hash}.md` (tenant/project as **safe segments**, temp-then-move publish, one core for both ingest paths) and records `version_actor` / `version_reason` / `content_hash` — the actor **derived server-side** (a client-supplied `version_actor` — like `source_retention` / `source_dropped` / `restores` — is stripped at the HTTP and connector boundaries), and `has_artifact` a **read + verified** claim (`artifact_state`: none · verified · unverified · missing · mismatch), never the pointer alone — memoized for `KB_VERSIONS_ARTIFACT_STATE_CACHE` seconds (default 300, `0` verifies on every read) so a timeline page does not re-read one object per row: only `verified` is cached, a repairable state is always re-read, the removal gate forgets the entry, and the content/diff endpoints never read the memo. Diff and the new `GET …/versions/{versionId}/content` read the artifact when there is one and say so (`from_source` / `to_source`, additive), so the UI labels a diff *faithful* or *index diff*; `restore` leaves the artifact in place, keeps the creation provenance immutable and records who restored (`restored_by` / `restored_at`, from `metadata.restores`). `KB_SOURCE_RETENTION` is wired for the first time (`markdown_only` drops the original after the artifact commit; `reference_only` stores nothing — not even an `.ocr/` run). Retention and erasure cover it (hard delete, `kb:prune-archived-versions` + temp/orphan sweeps); `kb:artifacts-backfill` fills history only on a hash match, under each row's own retention contract. The `markdown_only` drop and every row commit of the same storage key share one cache lock (`KB_CONVERSION_ARTIFACTS_SOURCE_LOCK_WAIT` / `KB_CONVERSION_ARTIFACTS_SOURCE_LOCK_TTL`; temp sweep age `KB_CONVERSION_ARTIFACTS_TMP_MAX_AGE` and the writer's temp lease `KB_CONVERSION_ARTIFACTS_TMP_LEASE` that keeps an in-flight temp from the sweep whatever its age), so a concurrent ingest can never commit a `full_copy` version between the reference scan and the delete; every artifact publish and removal runs under a per-path cache lock, so a lock-capable cache store (`CACHE_STORE=redis` in production) is a requirement of the feature — on a store that cannot lock, a publish is refused and a removal is reported `failed`, never run unguarded — the `null` store, which implements the lock contract but grants every lock, is detected and refused like a store that cannot lock at all; the residual is a provider whose locks do not exclude ACROSS processes (the array store). The timeline is bounded and paged on every surface (`KB_VERSIONS_TIMELINE_LIMIT`, default 100; `total` / `limit` / `offset` / `truncated` additive, a "Load older versions" control in the UI). Tri-surface (R44): `kb:doc-versions`, the HTTP endpoints, MCP `KbDocumentVersionsTool` (metadata only — the artifact is un-redacted converter output, a documented exception). See [Documents & OCR](https://padosoft.mintlify.app/documents-and-ocr) | v8.36 |
...
| Ingest provenance labels (ADR 0028 phase 1) | Ingestion recorded what a document *is* and nothing about who wrote it, so a wiki page written by staff and an email written by anyone who knows the address were stored as the same kind of fact — and both became retrieval grounding on a platform that also exposes MCP tools. A connector now declares a `ProvenanceTier` (`trusted-internal` / `untrusted-external` / `machine-generated`) through the opt-in `DeclaresProvenance` capability on `askmydocs-connector-base` ^1.5, resolved **per installation**; the host stores it on `knowledge_documents.provenance_tier` and reports corpus composition tri-surface (R44): `kb:provenance`, `GET /api/admin/kb/provenance`, MCP `KbProvenanceTool` (roster **48 → 49**). IMAP declares `UntrustedExternal` and does **not** soften it for a folder allow-list — that narrows which mail is ingested, never who could send it. Undeclared reads as the trusted default (every pre-existing document unchanged) while an *unrecognised* stored value fails **closed**. This phase labels only; enforcement is v8.34. Deliberately not the Auto-Wiki curation tier — a human-accepted page summarising an external email is both at once. See [ingest provenance](https://padosoft.mintlify.app/ingest-provenance) | v8.32 |
| Source permission mirroring (ADR 0028 phase 2) | A connector read a file from a system that already knew who could see it — Drive knows a document is shared with three people — and ingestion discarded all of it. From that moment the only gate was project membership, so a file shared with three became retrievable by everyone in the project, cited in answers to people the source never granted anything. No mistake was required; it was the default behaviour of a correct-looking ingest. A connector can now report that list through the opt-in `SupportsSourceAcl` capability on `askmydocs-connector-base` ^1.6; `SourceAclMirror` resolves it against the directory, mirrors what it can onto `knowledge_document_acl` (`origin='source-mirror'`), and **deletes rows the source no longer names** — a mirror that only adds is a slow leak, because a revoked share stays granted and then looks deliberate. Manual grants are never touched. **Not knowing is never treated as knowing**: a truncated, rate-limited, inherited or malformed list leaves the previous mirror in force rather than revoking access because a request failed. Restriction is recorded on `knowledge_documents.source_acl_enforced_at`, not inferred from mirrored rows — a complete list naming only people the app cannot place yields zero rows, and reading that as "unrestricted" would leave open precisely the documents whose readers are least likely to be colleagues. Enforced in `AccessScopeScope`'s SQL as well as the policy (**R33**): retrieval reaches chunks via `whereHas('document')` and never calls the Gate, so a policy-only arm still hands the document to the model as grounding. Unresolved principals are **queued, not dropped** — dropping one silently removes access from somebody who legitimately has it upstream. Tri-surface (R44): `kb:source-acl`, `GET/PATCH /api/admin/kb/source-acl`, MCP `KbSourceAclTool` (roster **49 → 50**, read-only by design — granting stays outside the agent trust boundary, ADR 0023/0024) + a **Source Permissions** admin screen. See [source permissions](https://padosoft.mintlify.app/source-permissions) | v8.33 |
| Provenance tool firewall (ADR 0028 phase 3) | Three reasonable facts compose into one problem: IMAP ingests email, which anyone who can send one writes; ingested content becomes retrieval grounding; and the same platform exposes MCP tools to the model. Nothing between them distinguishes a colleague's runbook from a stranger's instructions, so a paragraph in an inbound email reaches the model with the standing of an internal policy document — and if it says *"call the transfer tool"*, the model has no basis to decline. That is prompt injection with the entire chain inside one product, and no step of it is a bug. `ProvenanceToolFirewall` enforces the ADR's rule: an `untrusted-external` chunk may be **quoted** but must never **influence a tool call**. The asymmetry is the design — refusing to quote would break the product to fix the security problem, while refusing to act costs the turn its actions and nothing else. The answer still comes back from the same context with the same citations; only the tool loop is withheld. It never inspects content (detecting an injection by reading it is a losing game; refusing to act on what an outsider wrote is not), assesses **every** block the model can see including graph-expanded and rejected-approach chunks, and treats an undeclared tier as trusted so no existing deployment loses tools on upgrade. Both chat channels pass the verdict, so streaming is not a way around it, and an absent verdict reads as **allowed** — blocking is an explicit decision, never a decode accident. `KB_PROVENANCE_TOOL_FIREWALL` (**default ON** — a deliberate reversal of ADR 0028's `OFF`, recorded in the ADR: a security control that ships off protects nobody until someone remembers it. Upgrading a deployment that already ingests email means turns grounded in a message stop being able to act; `false` restores the previous behaviour exactly, and R43 covers both states); the policy is reported on all three phase-1 provenance surfaces (R27 additive). See [tool firewall](https://padosoft.mintlify.app/tool-firewall) | v8.34 |
...
| Scope allowlists enforced in retrieval (R33) | A membership's `scope_allowlist` (folder globs / tags) is pushed into SQL by `App\Support\ScopeAllowlistSql` + `AccessScopeScope`, so it reaches the RAG hot path — which resolves chunks through `whereHas('document')` and never calls `KnowledgeDocumentPolicy`. Previously that arm ran only in the policy, so a member scoped to `hr/policies/**` still received `hr/salaries/**` chunks as grounding, with citations (H8's shape, one arm later). Globs keep `KbPath` segment semantics via an exact separator count; tags become a tenant-correlated `EXISTS`; the two are OR'd; allowlists stay per-project. Memberships **without** an allowlist generate the identical query as before. The one glob shape mixing single- and cross-segment wildcards cannot be expressed exactly and therefore **fails closed** (grants nothing) rather than widening to a prefix match — see [security & threat model](https://padosoft.mintlify.app/architecture/security-and-threat-model) | v8.31 |
...
| 9 registered Flow definitions (saga / compensation) | `kb.ingest` (6-step) / `kb.canonical-index` (3-step) / `kb.promote` (4-step approval-gated, first use of `approval-gate` primitive) / `kb.delete` (4-step) / `kb.prune-deleted` / `kb.prune-embedding-cache` (conditional approval gate) / `kb.prune-chat-logs` / `kb.rebuild-graph` / `kb.ingest-folder` (3-step fan-out). Reverse-order compensation chains; persisted to `flow_runs` + `flow_run_nodes` + `flow_audit` + `flow_approvals` + `flow_webhook_outbox` (persistence opt-in, default-OFF). Host-owned tenant boundary: the package is tenant-agnostic by design, so the host implements the persistence contracts + `TenantScopedDashboardReads` itself. Tri-surface read (R44): `Flow::execute` / `flow:*` commands + the `/admin/flows` cockpit + the MCP `FlowRunStatusTool`, all over one read model — [doc-site](https://padosoft.mintlify.app/flow-orchestration), [migration runbook](docs/runbooks/flow-v2-migration.md) | v4.2 · v2 engine |
...
| Sister `padosoft/*` package stack | `laravel-ai-regolo` v1.2.1 (Regolo provider for `laravel/ai`, on the 0.8 line since v8.19) + `laravel-ai-finops` v1.4.0 + `laravel-ai-guardrails` v1.1.0 (+ `-admin` v1.0.0; AI safety firewall enforcing on chat since v8.19) + `laravel-pii-redactor` v1.2 (PII detection with EU country packs: Italy + Germany + Spain) + `laravel-pii-redactor-admin` v1.0.2 + `laravel-flow` v2.5 (saga engine + approval gates + webhook outbox + replay; run-node graph schema since v2) + `laravel-flow-admin` v2.4 + `eval-harness` v1.3 (golden datasets + 7 metrics + cohorts + adversarial + LLM-as-judge; since v8.18 a runtime `require` and the retrieval-metric source of truth via `PackageMetricAdapter`) + `eval-harness-ui` v1.0.0 — every package MIT, every architecture test enforces standalone-agnostic invariants (zero refs to `KnowledgeDocument` / `kb_*` tables / `lopadova/askmydocs` in `src/`); the whole stack runs on the `laravel/ai` **0.8** line (v8.19, ADR 0016) | v4.2 |
...
| **v8.36 → v8.39** | ✅ shipped (v8.36 W1+W2 merged to `main` 2026-09-16; v8.37 W3 merged to `main` 2026-09-20; v8.38 W4 merged to `main` 2026-09-26 via PR #511; v8.39 W5 merged to `main` 2026-09-26 via PR #512 — see the "Tags" note below). W6 (the "vision" column of the original plan) was left deferred at v8.39 closure and later **promoted and shipped as v8.40.0** — see the row below. | **Document intelligence + portable LLM Wiki export + delegated maintenance.** A competitor audit ([Annota AI](docs/v4-platform/AUDIT-2026-09-11-annota-ai-gap.md)) found the one seam where a data-preparation platform is ahead of us: *file → reviewed Markdown*. We had **no OCR** — `PdfConverter` read the text layer only and `SourceType` accepted no `image/*`, so a scanned contract arriving through IMAP or OneDrive was a 422 or an empty document — and the converted Markdown was **not stored** (`source_retention`/`markdown_path` were still the ADR 0014 schema foundation), so nothing could be reviewed or exported, and the v8.7 *Cloud Time Machine* diffed chunk reconstructions rather than documents. Five workstreams, each behind a default-OFF flag (R43), one release per week: **W1** (v8.36, **shipped** PR #478) `OcrConverter` with pluggable drivers (docling / Mistral OCR / vision-LLM / tesseract), figures to `images/`, LaTeX formulas, per-page confidence, extraction origin `ocr` on document and chunks (orthogonal to the ADR 0028 `provenance_tier`) + PII redaction before embedding + FinOps metering with a cost estimate on the upload modal — [ADR 0029](docs/adr/0029-v836-ocr-converter-drivers-and-ocr-provenance.md); **W2** (v8.36, **shipped** PR #479) stored conversion artifacts on the existing *Cloud Time Machine* (v8.7) — faithful diff/restore of the document instead of a chunk reconstruction, versions born on correction with `actor`/`reason`, and the MCP read surface the feature never got, `KB_SOURCE_RETENTION` wired for the first time (`markdown_only` / `reference_only`) — [ADR 0030](docs/adr/0030-v836-conversion-artifacts-on-the-time-machine.md); **W3** (v8.37, **shipped** PRs #494/#496/#497) *Digitization Review* — non-canonical OCR documents born in the `auto` tier (canonical frontmatter keeps its own say) and promoted to `human` on approval (ADR 0014 already models it), the reranker firewall's `human > auto` ordering now applying to non-canonical rows too, an MCP `KbProposeTextCorrectionTool` that **proposes** a text correction candidate and a human-only review/approve/reject status (the explicit inverse of Annota's `update_document_page`), a `ReviewTab` admin UI for per-page toggling and the correction queue — the *advanced* side-by-side original/Markdown pane with a confidence heat-map, the CER/WER metrics, and the `laravel-flow` workflow integration are **deferred past this GA**, documented in the [closure STATUS](docs/v4-platform/STATUS-2026-09-20-v837-w3-closure.md) — [ADR 0031](docs/adr/0031-v837-digitization-review-and-auto-tier-mapping.md); **W4** (v8.38, **shipped** PRs #503/#506/#509/#511, plus cross-cutting hotfix #507/#508) `kb:export-wiki`/`POST /api/admin/kb/exports` — the Karpathy layout (`raw/`, `wiki/`, `AGENTS.md`, `CLAUDE.md`, `MANIFEST.json`) but **already compiled**, tiered (`tier`/`evidence`/`provenance_tier`/`extraction` frontmatter, rejected-approaches included), ACL-filtered (R33), hash-manifested, with a `.mcp.json` back to the live server carrying no embedded credential, and `kb:import-wiki`/`KbImportWikiTool` returning edits as ADR-0003 promotion candidates, never a direct write — `llms.txt`/`llms-full.txt` and the `include_images`/`--format` variants are **deferred past this GA**, documented in the [closure STATUS](docs/v4-platform/STATUS-2026-09-25-v838-w4-closure.md) — [ADR 0032](docs/adr/0032-v838-portable-wiki-export-and-candidate-only-import.md); **W5** (v8.39, **shipped** PR #512) Auto-Wiki maintenance as an OPTIONAL `padosoft/laravel-routines` routine — `kb:wiki-maintain`'s existing cron gets a named, observable identity, tenant-scoped entirely by this app's own service (the package has no tenant concept), with the package's own generic admin API and webhook ingress both turned off by default — an independent review caught and fixed a cross-tenant gate-scoping leak and a TOCTOU provisioning race before merge; the mandate/consent step-up flow is **deferred past this GA**, documented in the [closure STATUS](docs/v4-platform/STATUS-2026-09-26-v839-w5-closure.md) — [ADR 0033](docs/adr/0033-v839-auto-wiki-maintenance-as-a-delegated-routine.md). **W6** (the original plan's "vision" column) was explicitly optional/deferred at v8.39 closure — the plan itself conditioned promotion on an image-heavy tenant signal that did not yet exist. [PLAN](docs/v4-platform/PLAN-v8.36-document-intelligence-and-llm-wiki-export.md) · [STATUS v8.36](docs/v4-platform/STATUS-2026-09-16-v836-w1-w2-closure.md) · [STATUS v8.37](docs/v4-platform/STATUS-2026-09-20-v837-w3-closure.md) · [STATUS v8.38](docs/v4-platform/STATUS-2026-09-25-v838-w4-closure.md) · [STATUS v8.39](docs/v4-platform/STATUS-2026-09-26-v839-w5-closure.md). **Tags:** this session's git credential can push branch commits and merge PRs but returns `403` on `git push` of a tag ref — `v8.36.0`, `v8.37.0`, `v8.38.0`, `v8.39.0` and `v8.40.0` are therefore **not yet tagged** even though all five cycles' code is merged to `main`; an operator with tag-push rights should run the `git tag -a … && git push origin <tag>` commands the STATUS docs describe. |
| **v8.40.0** | ✅ shipped 2026-09-26 (merged to `main` via PR #516) | **Vision column for Tabular Review — W6, promoted.** The signal the v8.39 closure was waiting on arrived the same day: fashion-ecommerce tenants whose catalogs are photo-heavy need exactly the case the plan named — colour/material/pattern extraction from product photos, reviewed by a human in the same Tabular Review grid every other column already renders in. A fourth `AgentKind` (`vision`, alongside `extract`/`graph`/`verify`): one vision-LLM call per document, over the document's OCR-extracted figures (`OcrFigureStore`, v8.36/W1) or, when none exist, the document's own file when it is itself an image — never both, capped at `KB_TABULAR_VISION_MAX_IMAGES` (default 4). Reuses the `SdkAnonymousAgent`/`Base64Image` calling pattern the `vision-llm` OCR driver already established (metered automatically, no new infrastructure). Rides the v8.19 agent-dimension plumbing for free: **zero new schema, zero new HTTP routes, zero new MCP tools** — `KbRunReportTool`'s existing column projection already passes a `vision` column through unchanged. Default **OFF** (`KB_TABULAR_VISION_ENABLED`, R43, both states tested): a vision column with the flag off produces a clean, definite red `failed` cell naming the flag, every other column in the same review unaffected. An independent subagent review (Copilot unavailable — out of budget for this roadmap; `@codex review` posted but never responded) caught and fixed one real crash path before merge: a malformed recorded storage prefix could throw uncaught out of the resolver, aborting every column for that document rather than degrading to the promised red cell — fixed by guarding with `StorageNamespace::prefixCanNamePath()`, the same pattern five other consumers already use. **Deferred, documented** ([ADR 0034](docs/adr/0034-v840-vision-column-in-tabular-review.md) §Deferred): manual cell-value override (editing a persisted cell without re-running extraction — cross-cutting across every agent kind, not vision-specific); region-level figure detection inside `vision-llm` OCR itself; a first-class multi-image "product asset" KB concept beyond one-photo-per-document. See [ADR 0034](docs/adr/0034-v840-vision-column-in-tabular-review.md) and the [Agentic Knowledge Reports doc-site page](https://padosoft.mintlify.app/agentic-knowledge-reports) (extended in place, not forked). PR #516. |
| **Future** | ⏳ planned for v8.x or v9.0 | Auto-Wiki follow-ups: navigator→chat wiring + benchmark-gated default-ON. Agentic Knowledge Reports follow-ups: SSE progressive-paint generate + the Glide canvas grid (deferred for per-cell testability/a11y); a manual cell-value override capability (v8.40 §Deferred). SSO / SCIM enterprise auth — surfaced by the v8.8 Affine gap audit; #1 Semantic Time Travel + #8 v2 (answer drift replay) — parked from v8.0; the v8.7 Time Machine and the v8.36 artifacts give it its data layer. |
...
|---|---|
| [`CLAUDE.md`](CLAUDE.md) | Authoritative project brief — what AskMyDocs is, critical components, schemas, flows, 44 codified review rules (R1–R33 + R36–R46; R34–R35 reserved), branching strategy (R37), Copilot review loop (R36) |
...
| `padosoft/laravel-flow` v2.5 | In-process saga / compensation engine + approval gates + webhook outbox + replay lineage | ✅ wired since v4.2 W2 — 9 Flow definitions registered | [github](https://github.com/padosoft/laravel-flow) |
| `padosoft/laravel-flow-admin` v2.4 | Blade + Alpine cockpit SPA — runs / approvals / webhook outbox / definitions | ✅ at `/admin/flows`. **Since v8.8.2 the host shows a native center-only panel** (KPI probe of `/admin/flows/api/live` + section cards) that launches the full Blade+Alpine cockpit in a new tab (`target="_blank"`) — so the cockpit is never nested inside the host chrome. **This supersedes ADR 0005's "flow-admin stays iframe-mounted" assumption**; what remains true from ADR 0005 is that the cockpit itself stays Blade+Alpine, not React | [github](https://github.com/padosoft/laravel-flow-admin) |
...
- Honour the 44 codified review rules (R1–R33 + R36–R46; R34–R35 reserved) — see [`CLAUDE.md`](CLAUDE.md) section 7
...
**v8.40.0 — Vision column for Tabular Review: a fourth `AgentKind` over product photos and OCR figures (W6, promoted; GA merged to `main` 2026-09-26 via PR #516).**
W6 of the v8.36 Document Intelligence plan was left an explicit, not-committed adjacency at v8.39 closure — its own text conditioned promotion on "the grid gets image-heavy tenants." That signal arrived the same day: fashion-ecommerce tenants whose catalogs are photo-heavy need exactly the case the plan named — colour/material/pattern extraction from product photos, reviewed by a human in the same Tabular Review grid every other column already uses. [ADR 0034](docs/adr/0034-v840-vision-column-in-tabular-review.md) is the plan addendum the promotion clause required before a branch could open. A fourth `AgentKind` (`vision`, alongside `extract`/`graph`/`verify`): `VisionColumnResolver` resolves images in a fixed precedence — OCR-extracted figures (`OcrFigureStore`, v8.36/W1; only the `mistral`/`docling` OCR drivers extract figures) first, then the document's own file when it is itself an image (the primary real-world shape for most fashion catalogs: one photo per SKU), then a definite red "no visual evidence" cell naming both sources checked, never a silent skip (R14) — capped at `KB_TABULAR_VISION_MAX_IMAGES` (default 4, SEC-LLM-001 gate 7). The provider call reuses the `SdkAnonymousAgent`/`Base64Image` pattern the `vision-llm` OCR driver already established (metered automatically by the `laravel/ai` SDK lifecycle hook, no double-counting, no new infrastructure). Because v8.19/W4 already built the `agent`-dimension plumbing generically, this fourth kind rides it for free: **zero new schema** (`tabular_reviews.columns_config` and `tabular_cells.content` already accommodate it), **zero new HTTP routes** (`Rule::in(AgentKind::values())` already validated the enum), **zero new MCP tools** (`KbRunReportTool`'s existing column projection passes `agent: "vision"` through unchanged, confirmed by test). Default **OFF** (`KB_TABULAR_VISION_ENABLED`, R43, both states tested): a vision column with the flag off produces a clean, definite red `failed` cell naming the flag — every other column in the same review generates normally, never a 500, never a silent fallback to `extract` semantics. An independent subagent review (Copilot unavailable for this roadmap — out of budget; the `@codex review` fallback comment posted but never responded) caught and fixed one real crash path before merge: the standalone-image-document fallback composed a storage path from a recorded prefix without first checking `StorageNamespace::prefixCanNamePath()` — a legacy/malformed prefix is returned *verbatim* by design, and every other consumer that composes a path from it (`IngestDocumentJob`, `OcrService`, `DocumentDeleter`, `DocumentIngestor`, `ReembedDocumentJob`) already guards against it; this resolver was the one that didn't, and an uncaught exception would have aborted *every* column for that document, not just degraded the vision one. Fixed with the same guard, proved by a regression test. Full PHPUnit + Vitest + the real Playwright E2E gate (never run before this PR — validated green on the first real run) all green. **Deferred, documented** ([ADR 0034](docs/adr/0034-v840-vision-column-in-tabular-review.md) §6/Deferred): manual cell-value override (editing a persisted cell without re-running extraction) is a materially larger, cross-cutting capability spanning every `AgentKind`, not vision-specific, and no column type has it today; region-level figure detection inside `vision-llm` OCR itself (only Mistral/Docling extract figures); a first-class multi-image "product asset" KB concept beyond one-photo-per-document. Doc-site coverage extends `agentic-knowledge-reports.mdx` in place (image-source precedence, router diagram, worked fashion-catalog example) rather than forking a new page, since vision is a fourth entry in a taxonomy that page already owns end to end (R9/R45). See [ADR 0034](docs/adr/0034-v840-vision-column-in-tabular-review.md) and [Agentic Knowledge Reports](https://padosoft.mintlify.app/agentic-knowledge-reports). PR #516.

**v8.39.0 — Auto-Wiki maintenance as a delegated routine: `kb:wiki-maintain` gains an optional `padosoft/laravel-routines` identity (W5 GA merged to `main` 2026-09-26 via PR #512 — see the "Tags" note in [STATUS](docs/v4-platform/STATUS-2026-09-26-v839-w5-closure.md)).**
Closes the fifth and last Document Intelligence workstream planned at v8.36: `kb:wiki-maintain`'s existing nightly cron (index rebuild + lint + backfill un-enriched docs) can now optionally run as a named, observable **routine** instead of an anonymous scheduler entry, via `padosoft/laravel-routines` v1.2.0 — a real agentic-routine engine (schedule, dispatch, mandate/consent, budget, escalation) added as a `require` Composer dependency, the same convention as every other `padosoft/*` package here (ADR 0033 corrects the plan's original "optional dependency" framing once the real package was installed and its source read: "optional" means the *feature* you may not enable, never the *package* you may not have). `WikiMaintenanceRoutineTarget implements RoutineTarget` — a thin adapter over the UNCHANGED `WikiMaintainer` core. `WikiRoutineService` is the one core behind the tri-surface (`kb:wiki-routine {status|run}`, `GET/POST /api/admin/kb/wiki-routine[/run]`, MCP `KbWikiRoutineStatusTool` read-only), auto-provisioning a `Routine` row on first use with the package's generic `organization_id` column repurposed to hold this app's `tenant_id` — the engine has no tenant concept at all, so R30 scoping is entirely this service's job. `WikiMaintenanceRoutineGate` decides whether the legacy cron slot stays registered, failing safe to "keep the cron" on any database error. The package's own generic admin API and its signed webhook ingress are both turned **off** by default (`ROUTINES_API_ENABLED=false`, `ROUTINES_WEBHOOKS_ENABLED=false`) — the admin API has no tenant boundary and defaults to cross-tenant read access, and nothing this app creates is a webhook-triggered routine. An independent subagent review (R36 fallback — neither Copilot nor `@codex review` responded) caught and fixed two real issues before merge: the gate's "an active routine exists" check was originally un-scoped by tenant, so any OTHER tenant enabling the routine would have silently disabled the shared cron slot `default` still depends on; and the routine-provisioning find-then-create had no lock, a TOCTOU race that could double-provision a routine under concurrent triggers — both fixed with regression tests proving the actual invariant, not just sequential idempotency. Every mutating surface is default-**OFF** (`KB_WIKI_ROUTINE_ENABLED`, R43, both states tested). **Deferred past this GA** (documented, not silently dropped): the mandate/consent step-up flow — the engine's `MandateExceeded`/pause-and-ask machinery is real and wired through, but nothing this cycle can trigger it — and W6, the original plan's optional "vision" column, which this closure does not promote into a committed workstream. See [STATUS](docs/v4-platform/STATUS-2026-09-26-v839-w5-closure.md), [ADR 0033](docs/adr/0033-v839-auto-wiki-maintenance-as-a-delegated-routine.md) and [Auto-Wiki](https://padosoft.mintlify.app/auto-wiki). PR #512.

**v8.38.0 — Portable Wiki Export + candidate-only import: `.mcp.json`, frontmatter round-trip, `kb:import-wiki` (W4 GA merged to `main` 2026-09-26 via PR #511 — see the "Tags" note in [STATUS](docs/v4-platform/STATUS-2026-09-25-v838-w4-closure.md)).**
Closes the second gap the [Annota AI audit](docs/v4-platform/AUDIT-2026-09-11-annota-ai-gap.md) named: a portable, agent-readable Markdown wiki with a live connection back to the source, none of which AskMyDocs had before this cycle. **W4a** (PR #503) ships the synchronous `kb:export-wiki` CLI core — an ACL- and PII-scoped `wiki/`+`raw/` folder computed by authenticating as the `--as-user` for the export's duration, so `AccessScopeScope` gates the document query itself, plus a `MANIFEST.json` sha256 hash chain for corruption detection. **W4b** (PR #506) adds the idempotent async HTTP surface (`POST /api/admin/kb/exports`, `ExecuteKbWikiExportJob`, download re-authorization against the export's recorded document ids, `kb:prune-wiki-exports` hourly sweep). A cross-cutting security hotfix (#507 direct to `main`, ported via #508) fixed the MCP transport binding the wrong principal to a request — found while building W4c's MCP tools, needed before they could be trusted. **W4c** (PR #509) closes the loop: `.mcp.json` in every exported folder (placeholder credential env-vars only, never an embedded token), the exported frontmatter round-tripping through `CanonicalParser`, and `KbWikiImportService` — candidate-only re-import (ADR 0003: an edit proposes a `PromotionFlow` run, never a direct write) via `kb:import-wiki` (bulk, local folder), `POST /api/admin/kb/imports` and `KbImportWikiTool` (single document), all three delegating to one core. An independent subagent review (R36 fallback — neither Copilot nor `@codex review` responded across two rounds) caught and fixed two real issues before merge: a CWE-22-adjacent path-containment bypass in `importFolder()`'s symlink/sibling-directory check, and a frontmatter-only edit (body unchanged) being silently dropped as "unchanged." Every surface is default-**OFF** (`KB_WIKI_EXPORT_ENABLED` gates export AND import — no separate import flag, R43, both states tested). **Deferred past this GA** (documented, not silently dropped): `llms.txt`/`llms-full.txt`, and the `include_images`/`--format` export variants (explicitly rejected rather than ignored). See [STATUS](docs/v4-platform/STATUS-2026-09-25-v838-w4-closure.md), [ADR 0032](docs/adr/0032-v838-portable-wiki-export-and-candidate-only-import.md) and [Portable Wiki Export](https://padosoft.mintlify.app/portable-wiki-export). PRs #503, #506, #507, #508, #509.

**v8.37.0 — Digitization Review: the `auto`→`human` review loop for OCR'd content (W3 merged to `main` via PR #499 2026-09-20; the `v8.37.0` tag itself is pending — see the "Tags" note in [STATUS](docs/v4-platform/STATUS-2026-09-20-v837-w3-closure.md)).**
Wires the existing `generation_source` tier (`human`/`auto`, ADR 0014) to what OCR (v8.36/W1) produces: a converted page is born `auto` until a person approves it, closing the review-workflow gap the [Annota AI audit](docs/v4-platform/AUDIT-2026-09-11-annota-ai-gap.md) named. **W3a** (PR #494) ships the schema (`kb_document_page_reviews`, `kb_text_correction_candidates`, both R30/R31 tenant-aware), the `Reranker::canonicalAdjustment()` firewall fix (the `human > auto` anti-hallucination ordering previously short-circuited to zero for every non-canonical chunk, so an unreviewed and a reviewed scan ranked identically), and `KbReviewService`'s read/approve core with its PHP/CLI + HTTP + MCP-read tri-surface. **W3b** (PR #496) adds the agent-proposed correction-candidate flow — `KbProposeTextCorrectionTool` writes only a *candidate*, never the corpus, per ADR 0003's boundary; approval is a three-phase, R21-atomic flow that locks the candidate + document, re-validates the correction text against the document's CURRENT version, and mints a genuinely new version through `DocumentIngestor::reembedFromMarkdown()`. **W3c** (PR #497) ships the `ReviewTab` admin UI and, as scope surfaced by its own Copilot review loop, four unrelated pre-existing MCP transport security fixes (rate-limiter ordering and keying, per-method token validation, a `TenantContext` leak across requests on the same Octane worker). Every mutating entry point is default-**OFF** (`KB_DIGITIZATION_REVIEW_ENABLED`, R43, both states tested); MCP tool *registration* is flag-independent by design (`{disabled: true}`, never a 500 or a vanishing manifest entry). **Deferred past this GA** (documented, not silently dropped): the side-by-side original↔Markdown review pane with a confidence heat-map, CER/WER quality metrics, and `laravel-flow` workflow integration for the review queue. See [STATUS](docs/v4-platform/STATUS-2026-09-20-v837-w3-closure.md), [ADR 0031](docs/adr/0031-v837-digitization-review-and-auto-tier-mapping.md) and [Digitization Review](https://padosoft.mintlify.app/digitization-review). PRs #493, #494, #496, #497.

**v8.36.0 — Document intelligence W1+W2: OCR ingestion and conversion artifacts on the Time Machine (W1+W2 merged to `main` via PR #492 2026-09-16; the `v8.36.0` tag itself is pending — see the "Tags" note in [STATUS v8.37](docs/v4-platform/STATUS-2026-09-20-v837-w3-closure.md)).**
Closes the seam a competitor audit ([Annota AI](docs/v4-platform/AUDIT-2026-09-11-annota-ai-gap.md)) found: *file → reviewed Markdown*. Before this cycle there was no OCR (`PdfConverter` read only the text layer, `SourceType` accepted no `image/*`) and converted Markdown was never stored (`source_retention`/`markdown_path` were still the ADR 0014 schema foundation), so a scanned document produced an empty ingest and nothing could be reviewed or exported. **W1** (PR #478, [ADR 0029](docs/adr/0029-v836-ocr-converter-drivers-and-ocr-provenance.md)) adds `OcrConverter` with pluggable drivers — `tesseract` (default, local), `docling` (local, best layout/tables), `mistral-ocr` and `vision-llm` (remote, gated behind `KB_OCR_ALLOW_REMOTE` — the bytes leave the tenant before the PII seam sees the text) — content-addressed run reuse keyed on bytes × driver × figure switch so an identical re-ingest never pays twice, figures extracted to `images/`, per-page confidence, and PII redaction running on the OCR output before embedding exactly as it does on any other source. **W2** (PR #479, [ADR 0030](docs/adr/0030-v836-conversion-artifacts-on-the-time-machine.md)) stores the exact Markdown every version was chunked from at `.artifacts/{tenant}/{project}/{source}.versions/{version_hash}.md`, so the v8.7 Cloud Time Machine diffs and restores the *document* instead of a chunk reconstruction, and wires `KB_SOURCE_RETENTION` for the first time (`markdown_only` drops the original once every referencing version has an artifact; `reference_only` stores nothing, not even an `.ocr/` run). Both flags default **OFF** (R43, both states tested). The review cycle (round 1 → round 51, 0 must-fix outstanding) surfaced and closed a family of TOCTOU races around the artifact/source lifecycle — a `markdown_only` drop racing a concurrent reader, an OCR run purge racing a fresh ingest still converting it, a `kb:artifacts-backfill` reconversion racing the same drop — each closed with the same acquire-and-hold shape: reserve, then re-assert the reservation is still held immediately before the irreversible step, never probe-then-act. Two of the later rounds: `kb:artifacts-backfill`'s reconversion wrote the artifact without recording which OCR run it had just created or reused, so a later prune could delete that run out from under the artifact this very backfill had published — the reconversion now persists `converted->extractionMeta` onto the row atomically with the pointer, mirroring the primary ingest path; and `ReembedDocumentJob` read its row as `active` up front, then spent unlocked time (a disk read, chunking, embedding) before ever writing, so a concurrent delete in that window was non-durable — the write now re-checks the exact row under `lockForUpdate()` inside the same transaction, refusing rather than resurrecting or recreating it. Tri-surface throughout (R44): CLI (`kb:ocr`, `kb:doc-versions`, `kb:artifacts-backfill`), HTTP (upload estimate, version content/diff/restore), MCP (`KbDocumentVersionsTool`, metadata only — diff/content are documented exceptions since un-redacted converter output should never reach the model as a *tool result*). See [STATUS](docs/v4-platform/STATUS-2026-09-16-v836-w1-w2-closure.md) and [Documents & OCR](https://padosoft.mintlify.app/documents-and-ocr). PRs #477, #478, #479.
...
plus a `markdown_path` artifact column — the ingest wiring that acts on the mode
and writes the markdown artifact did not land in v8.11.x; it shipped in v8.36 (W2,
ADR 0030, behind `KB_CONVERSION_ARTIFACTS_ENABLED`);
