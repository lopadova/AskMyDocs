...
| **Future** | ⏳ planned for v8.x or v9.0 | Auto-Wiki cycle continues: source-retention wiring (save the converted markdown artifact), agentic graph-navigation retrieval (benchmarked), cross-model review + contradiction/novelty, apply-from-UI + opt-in auto-apply, scheduled wiki maintenance, admin Wiki Explorer/Health. SSO / SCIM enterprise auth + content export/portability — surfaced by the v8.8 Affine gap audit; #1 Semantic Time Travel + #8 v2 (answer drift replay) — parked from v8.0 |
