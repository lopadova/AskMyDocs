...
| **Future** | ⏳ planned for v8.x or v9.0 | Auto-Wiki cycle continues: graph canonicalization (cross-refs→`kb_edges`) + source-retention wiring, concept-page synthesis, wiki lint + per-tenant index hub, agentic graph-navigation retrieval (benchmarked), apply-from-UI + opt-in auto-apply, admin Wiki Explorer/Health. SSO / SCIM enterprise auth + content export/portability — surfaced by the v8.8 Affine gap audit; #1 Semantic Time Travel + #8 v2 (answer drift replay) — parked from v8.0 |
