...
| **v8.0** | 🚧 W3 shipped 2026-05-20 (rc3) — W4..W8 in flight | Killer-features cycle (8 weekly milestones). **W1 — Notification System core** ✅ shipped 2026-05-19 (PRs #188 W1.1 schema, #189 W1.2 dispatcher, #190 W1.3 in_app+email channels, #191 W1.4 bell SPA + `/app/admin/notifications` panel, #192 W1.5 `notifications:prune` cron + 90d retention env). Three new tenant-aware tables (`notification_events` + `notification_preferences` + `notification_digests` per ADR 0012), R31-compliant (BelongsToTenant + composite uniques start with `tenant_id`), R30 cross-tenant isolation enforced on every endpoint including mutations + DELETE, R21 atomic mark-read + dismiss with COALESCE, R14 presenter strips forensic `channel_dispatch_log` + `tenant_id` + `user_id` from the FE feed, HMAC-signed one-click email unsubscribe. Scheduler slot 13 (`notifications:prune` 04:10, default 90d retention via `NOTIFICATIONS_RETENTION_DAYS`; set 0 to disable). **W2 — Notification channels + preferences + Tier-1 scheduler env** ✅ shipped 2026-05-20 (PRs #195 W2.1 Discord/Slack/Teams/Webhook + shared `NotificationEventLogger` atomic helper, #196 W2.2 per-user preferences grid at `/app/admin/notifications/preferences`, #197 W2.3 per-tenant admin defaults at `/app/admin/notifications/defaults` + `NotificationPreferencesInitializer` seeder hook, #198 W2.4 env-tunable scheduler with `App\Scheduling\TierOneSchedulerRegistrar` + 24 commented-out `SCHEDULE_*` env vars + scheduler-status ops widget honouring config overrides). New `notification_tenant_defaults` table; PUT super-admin-only via in-controller `hasRole` check; React `canMutate` gates the UI so admins see a read-only view instead of bouncing off the 403. Composite gates (`EVAL_NIGHTLY_ENABLED` / `AI_ACT_REGULATORY_FEED_ENABLED`) exposed as `config('askmydocs.composite_gates.*')` so bootstrap + ops widget stay consistent under `php artisan config:cache`. **W3 — Why-not-cited + Counterfactual** ✅ shipped 2026-05-20 (PRs #200 W3.1 runner-up retrieval with tenant-safe semantic/FTS scope + uniform response shape, #202 W3.2 chunk feedback endpoint `POST /api/kb/feedback`, #203 W3.3 FE runner-up feedback actions + W3.5 FE counterfactual panel/toggle, #201 W3.4 counterfactual retrieval backend hardening). Counterfactual retrieval knobs are env-wired as `KB_COUNTERFACTUAL_ENABLED`, `KB_COUNTERFACTUAL_MAX_NEIGHBORS`, `KB_COUNTERFACTUAL_PER_PROJECT_LIMIT`, `KB_COUNTERFACTUAL_MIN_SIMILARITY`, `KB_COUNTERFACTUAL_CACHE_TTL_SECONDS`. Remaining in cycle: **W4** Decision-Debt Heatmap (#2) + Tier-2 per-tenant scheduler, **W5+W6** Living Collections (foundation + semantic), **W7** MCP-as-KB-Debugger (#9), **W8** Compliance Differential Pack v1 (#8) + GA close. Plan: [`docs/v4-platform/PLAN-v8.0-killer-features.md`](docs/v4-platform/PLAN-v8.0-killer-features.md). ADRs: 0012 (notifications)..0018 (compliance diff). |
