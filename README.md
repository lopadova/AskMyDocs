...
| **v8.11.1** | ✅ shipped 2026-06-13 | **Auto-Wiki Compiler** (Karpathy LLM-Wiki "ingest"). After a document is ingested, the async `AutoWikiCompiler` (dispatched by `IngestDocumentJob`, gated by `AutoWikiGate`, version-idempotent) asks the LLM to auto-enrich its frontmatter — topical `tags`, a tight `summary`, `aliases`, and `cross_references` (slug + why + edge_type) to its closest existing neighbours — and merges them into `frontmatter_json._autowiki`, marking the doc `generation_source='auto'`. **Firewall:** it NEVER touches a human-curated canonical doc (the authoritative tier keeps its gate); audited to `kb_canonical_audit` (actor `system:autowiki`). Uses the dedicated model override (`KB_AUTOWIKI_AI_PROVIDER`/`_MODEL`) when set, else the default chat provider. Default-ON, R43 both-states. +12 PHPUnit. |
| **Future** | ⏳ planned for v8.x or v9.0 | Auto-Wiki cycle continues: concept-page synthesis + canonicalization-into-auto-tier, wiki lint + per-tenant index hub, agentic graph-navigation retrieval (benchmarked), apply-from-UI + opt-in auto-apply, admin Wiki Explorer/Health. SSO / SCIM enterprise auth + content export/portability — surfaced by the v8.8 Affine gap audit; #1 Semantic Time Travel + #8 v2 (answer drift replay) — parked from v8.0 |
