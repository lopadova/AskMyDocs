...
- **Images** (`png`/`jpeg`/`tiff`/`webp`) become documents through `OcrConverter`; **scanned PDFs** stay with `PdfConverter`, which probes the text layer (`KB_OCR_PROBE_PAGES` × `KB_OCR_PROBE_MIN_CHARS`) and delegates to `OcrService` when it is empty or unreadable. `PdfConverter` is the only PDF match — no overlapping `supports()` (R23).
