...
- **Not yet shipped** (W4c, ADR 0032 §4/§7): `.mcp.json` (pending two MCP auth-chain fixes the ADR calls out), `llms.txt`/`llms-full.txt`, `include_images` and `--format` variants (both explicitly rejected rather than silently ignored by the current async request service), the MCP tools (`KbCreateExportTool`/`KbGetExportTool`/`KbImportWikiTool`), and `kb:import-wiki`'s candidate-only round-trip (ADR 0003 boundary).
- **Config**: `KB_WIKI_EXPORT_ENABLED` (the gate for every surface), `KB_WIKI_EXPORT_RETENTION_HOURS=24` (drives both the DB row's `expires_at` and the hourly sweep), `KB_WIKI_EXPORT_QUEUE=default` (queue name for `ExecuteKbWikiExportJob`).
...
**v8.37.0 — Digitization Review: the `auto`→`human` review loop for OCR'd content (W3 merged to `feature/v8.37` 2026-09-20; the `v8.37.0` tag itself is pending — see the "Tags" note in [STATUS](docs/v4-platform/STATUS-2026-09-20-v837-w3-closure.md)).**
